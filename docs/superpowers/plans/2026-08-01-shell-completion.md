...
- [ ] **Step 2: Implement.** Attach the filter completer to the filter positionals of `list`, `export`, `watch` — the same three `argv::unescape` already lists at `lib.rs:248`. Reuse that list rather than restating it if the shape allows. **Build every one of them with `complete::escaped_word_completer`, never with a bare `ArgValueCompleter`.** A bare one compiles, reads correctly, and is handed the escaped sentinel where the user typed `-ne`, so every tag exclusion returns nothing while `+tag` and `project:x` keep working — four of five prefixes, which looks finished. `complete::tests::escaping_drift` fails the build for it: it drives each attached completer with an escaped probe word and requires the restore. `candidates_for`'s fixture will also refuse to install itself over the new attachment; rewrite the two `a_partial_tag_exclusion_*` tests to drive the shipped provider against a seeded store rather than deleting that assertion.
