...
> **Renumbered on merge.** This section proposed itself as "D27" while the branch
> was unmerged. §12 had already assigned D27 to "an unrecognised filter token is
> an error, not an always-true term" long before this branch was cut, so the
> proposal was never going to land under that number. Landed as **D48** on
> 2026-07-29. The habit worth keeping: a decision number is only free once
> checked against `main`, not against the branch you happen to be standing on —
> this branch's own copy of `DESIGN.md` was stale at D41, and numbering from it
> would have collided with D42 instead.
