...
| **Watch / live mode** | `tasqx watch` draws into the alternate screen, viewport-bounded to the hottest rows, and live-updates from daemon `event` pushes — a zero-config "situation room". Reuses `render::task_table` rather than a second, ratatui-widget rendering path (**D96**). | daemon `event` stream |
...
### D96 — `watch`'s TTY path is alt-screen and viewport-bounded, not a second ratatui renderer
