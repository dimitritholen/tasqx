...
**Prefer an id range over a second index.** `events.id` is UUIDv7 and the PRIMARY KEY, which is why `ORDER BY id DESC` is already the newest-first ordering, so a time bound can be expressed against the existing key. An `idx_events_ts` is the fallback if that turns out not to be exact, not the opening move: a second index is written on every event insert, and every mutation in this store writes an event.

**Counting `reopen` changes `tasqx chart burndown`, which is why it is a ruling.** `chart::burndown` documents its own simplification — reopen events ignored, first close wins — and as a flagged approximation in a command run on demand that was a fair trade. A dashboard redraws the same series continuously, so a task closed, reopened and still open reads as permanently done on a screen the user checks to find out whether the pile is emptying. Correcting it is not an internal fix: the existing command's output moves, its tests move with it, and a documented approximation is being withdrawn. Fixtures cover add, done, cancel, reopen, import, a task predating the window, and several lifecycle cycles.
