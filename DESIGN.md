...
### D196 — Memory search is hybrid: a static embedding model built into the binary ranks by meaning beside FTS5's bm25, fused by reciprocal rank, every hit saying how it was found (tasks #835–#839; amends D41, extends D69, D136, D193)

**Decision.** `memory.search` gains a semantic retriever and, by default, fuses it with the lexical one D41 built. The pieces:

- **Model.** MinishLab's `potion-base-8M` (Model2Vec, MIT; vocabulary from bge-base-en-v1.5, MIT): a 29,528 × 256 static embedding table. It is stored in the tree as a per-row int8 matrix with one f32 scale per row plus its WordPiece vocabulary (about 7.8 MB together), built from a pinned upstream revision by `scripts/embedding-model.py`, which checks the download's SHA-256, and compiled into `tasqx-core` with `include_bytes!`, parsed once per process on first use. `NOTICE` carries the attribution. The model id `potion-base-8M@<revision>/int8` is a constant; every stored vector records it.
- **Inference is ours, in pure Rust.** A BERT-style WordPiece tokenizer (clean control characters, space around CJK, lowercase, strip accents via NFD, split on whitespace and punctuation, greedy longest-match `##` subwords, a word over 100 characters is `[UNK]`), then the mean of the token rows, L2-normalised. `[UNK]` and special tokens are dropped; text with no known token has no vector. Nothing is truncated: long text is chunked instead. A golden test pins the tokenizer against the upstream `tokenizers` output for a committed sample, so a hand-written tokenizer cannot drift from the one the table was trained with. No crate beyond `unicode-normalization` is added, and nothing native.
- **Chunks.** An annotation is one chunk unless it is over 150 words. A doc is split on blank lines and headings into chunks of about 40–150 words: short neighbours merge, a long paragraph splits on sentence ends with one sentence of overlap. Every chunk of a doc is prefixed with the doc's title and its heading path, so a chunk that only makes sense under its heading still carries it. An entry scores as its best chunk.
- **Storage and freshness.** Vectors live in `memory_vectors(kind, owner_id, chunk, model, text, vec)`, `vec` 256 little-endian f32, primary key `(kind, owner_id, chunk)`. Pure-SQL triggers delete an owner's vectors when a doc's `title`, `body` or `search_body` changes, when an annotation's `body` or `id` changes, and when either row is deleted; inserts do nothing. Before any search that uses meaning, the engine embeds every doc and every non-empty annotation that has no vector under the current model (a vector under another model is deleted first) and writes them in one transaction. So every write path — `memory.add/import/update/refresh/remove`, `annotation.add/update/remove`, undo, `store.import` in both modes, D191's fold, a recurrence spawn — and every writer — the daemon, an in-process CLI, each MCP server, an older tasqx binary on the same file — leaves the index correct without knowing it exists, because the only thing any of them has to do is the SQL write it already does. If that write cannot happen (a read-only file, a busy writer past the timeout), the missing vectors are computed in memory for this call only: a search never fails because of the vector index. Vectors are derived data: `store.export` does not carry them and `store.import` never needs them.
- **Search.** `memory.search` takes `mode`: `hybrid` (the default), `lexical` or `semantic`. `raw: true` is lexical by definition and is refused beside `mode: semantic` or `hybrid`. The lexical list is exactly D41/D69/D193 (all words, then the relaxed any-word OR), its top 50; the semantic list is the cosine of the query's vector against every chunk in the requested `scope`/`project`/`include_unscoped`, an entry's best chunk, kept only at or above the similarity floor, its top 50. The two are fused by reciprocal rank, score = Σ 1/(60 + rank), ties broken by id. The floor is `memory.semantic_floor` in config, default 0.30, between 0 and 1: static embeddings put unrelated English text at about 0.1–0.2 and related text at 0.14–0.48, so a query about nothing stored returns no hits instead of its nearest neighbours.
- **Every hit says why it is there.** Each hit gains `via` (`lexical`, `semantic` or `both`) and `similarity` (the cosine of its best chunk, three decimals, present whenever the entry has a vector). A semantic-only hit's `snippet` is its best chunk. The result keeps `matched` and `relaxed` for the lexical side and gains `semantic`: `{model, floor}` when the semantic side ran, `null` when it did not (`mode: lexical`, `raw`, or a query with no known token). `total` counts the fused set. `rank` stays D154's opt-in and is the fused score.
- **Where else.** The brief's and `start`'s derived memory (D136, D186) run hybrid with the task's title, tags and project as the meaning query and keep D147's half-for-docs split. `graph.query`'s title-word edges stay lexical: an edge claims the entry has the title's words. `tasqx memory search` takes `--mode` and marks a semantic-only hit; the MCP tool takes `mode`; the tool text, the server instructions, the in-binary method docs, the wiki and the starter prompt say that search matches meaning as well as words.

**Why now, and why this model.** D41 ruled out embeddings for three reasons: a runtime (`ort`) that multiplies the binary and the dependency tree, a model download at first use that breaks offline and no-signup use, and no standard way for clients to supply vectors. A static embedding model answers all three: there is no network, because the table is in the binary; there is no runtime, because inference is a lookup and a mean; and it adds one crate. What it costs is size, about +8.3 MB on a ~10.6 MB binary, which this ruling accepts: memory that finds "login" when the note says "sign-in", or the release process when an agent asks when to ship, is what the organiser is for, and #826 showed that agents' queries miss on words even when the answer is stored. potion-base-8M beat potion-retrieval-32M on a measured sample at a quarter of the size; transformer models (MiniLM, bge-small) retrieve better but bring back the runtime and cost milliseconds per text instead of microseconds.

**Why hybrid and not semantic alone.** Exact words — ids, file names, error strings, `D41` — are what bm25 is good at and static embeddings are bad at; meaning is the reverse. Reciprocal rank fusion needs no score calibration between the two, and it keeps D69's rule that every result can be explained: `matched` explains a lexical hit, `via` and `similarity` a semantic one. D147 rejected a rank boost because nothing in the result could explain it; this ranking puts its reasons on every hit.

**Why triggers that only invalidate.** A trigger cannot embed (SQL has no model), a function registered on the connection would make every older binary's writes fail with "no such function", and a cache in one process misses the others' writes. Deleting on change and embedding what is missing at search time is the one design under which a writer that has never heard of vectors still leaves them right. Embedding is microseconds per chunk, so the catch-up after a large import is a fraction of a second.

**Known limits.** The model is English: Dutch text matches mostly where query and note share words. Absolute similarity separates weakly with static embeddings, so the floor trades some paraphrase recall for silence on unrelated queries; it is configurable, not hidden. `similarity` can differ in the last digits between CPU architectures; order does not, because ties break by id.

**Rejected:** `fastembed`/`ort` and `candle` transformer models (runtime, size, per-text cost; D41's reasons stand for them); `model2vec-rs` (55 new crates including a C++ build step through `esaxx-rs`, and it truncates at 512 tokens silently); `sqlite-vec` (a C extension, pre-1.0, and brute force anyway at these sizes); the multilingual potion model (128 MB even as int8); cutting dimensions to shrink the table (measured top-1 loss).
