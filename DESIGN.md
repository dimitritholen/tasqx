...
`task.brief` (**D136**) and `report.outcomes` (**D137**) have shipped and are in the table above's namespace lists. Ruled and not yet built: `check.add` / `check.set` / `check.remove` (**D138**). All three rulings are additive under the frozen `"tasqx":"1"` API — new methods and new params on `task.done`, no existing response field renamed or removed (D56). D139 adds no method at all: `budget_tokens` is a field on `task.add` / `task.modify` and a pair of derived keys on `task.get`.
...
| `tasqx_brief_task` | R | `ref`, `memory_limit?`, `include_json?` | `{task, neighbourhood, memory}` (D136) — `task` is `task.get`'s own result; `neighbourhood.depends_on` carries each prerequisite's NEWEST annotation and `blocks` carries title and status only; `memory` is a `memory.search` result under an expression tasqx DERIVES from the task's title, tags and project, echoed in `matched` | `task.brief` |
