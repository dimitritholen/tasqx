...
| `tasqx_summary` | R | `group_by`, `filter?`, `metrics[]`, `all?`, `since?`, `until?` | grouped counts/estimates/overdue, plus the `filter`/`all` it applied (D69) and the `since`/`until` window it applied to `tracked_total`/the token buckets, independent of `filter`'s completion-date terms (D79) | `report.summary` |
...
### D79 — `report.summary` gains `since`/`until`, a window on WHEN the work happened, separate from `completed.*`'s WHEN IT CLOSED (tasqx audit-2026-09 #190, #224)
