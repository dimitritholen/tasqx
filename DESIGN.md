...
| **Scriptable & honest** | Every command speaks human-readable text *and* `--json`; exit codes and errors are stable contract, not decoration. |
...
3. **Honest under the hood.** Every command is a thin translation to one API `method`. Add `--json` for the raw envelope `result`; exit codes mirror the error model (§4).
...
`tasqx-mcp` ships as a thin binary (or `tasqx mcp` subcommand). It authenticates as a plugin with `permissions=["task.read","task.write","project.write"]` (§6a) — the *same* capability model that fences a Slack bridge fences the AI. An operator can run a read-only agent by handing it a plugin token lacking `task.write`. No AI-specific enforcement code exists; it's the extensibility model pointed at an LLM.
...
| **Plugins & hooks** | §6 (6a plugin API, 6b hooks + custom subcommands) | **Not built.** The capability/permission model it needs already exists and is proven in production by `tasqx mcp serve`, which authenticates as a scoped plugin (read/write scoping that fails closed — D7). Shipping a plugin loader now would freeze an ABI before there is a second real consumer to freeze it against; MCP is that consumer's dress rehearsal. |
...
### D7 — MCP: bundled `tasqx mcp` subcommand, scoped-token provisioning
**Decision:** Ship the server **inside the main binary as `tasqx mcp serve`** (no separate artifact). Provision with `tasqx mcp token --scope read|write`, which mints a scoped plugin token (§6a capability model); the host config invokes `tasqx mcp serve --token <t>`.
**Why:** One binary = one version to reason about and the simplest install, which matters most for adoption; scope lives in the existing plugin-permission model, so a read-only agent is just a token lacking `task.write` — no AI-specific enforcement code.
...
**Decision:** `Filter::parse` returns `Result`. A token the D8 grammar does not recognise is a `bad_request` naming the token and listing the shapes that would have worked; so are a dangling operator (`+api or`), an unclosed `(`, and a stray `)`. The empty filter is unchanged and still matches everything — no filter means no filtering. Unknown *values* are also unchanged: `status:pendign` parses and simply matches no row.
