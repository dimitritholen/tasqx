...
**What stays exactly as ruled.** `task.done`'s params and result are untouched — no new field, no altered semantics, D56's frozen shape holds. `tasqx api` and `tasqx done` see no `view` and behave exactly as before. The card drawn here is exactly what `tasqx_get_task view: "card"` would draw for the same task at the same page defaults, not a second renderer. No budget fitting runs on this path, and none is needed: the card is fixed 72-column geometry, a few KB at most against `RESPONSE_BUDGET_BYTES`, and the JSON beside it is the completion outcome, not a history page — there is no page to cut. A failed read-back degrades to the plain JSON answer, so presentation can never make a completion that happened look like one that did not.
