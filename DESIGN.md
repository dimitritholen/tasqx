...
### D135 — A memory doc's leading frontmatter is flattened to prose at `memory.add`/`memory.update` time, not read specially per screen (task #12)

**Decision:** `memory.add` and `memory.update` run a doc's `body` through `tasqx_core::frontmatter::flatten` before it ever reaches storage. A leading `---\n...\n---\n` block becomes plain `key  value` lines (D121(e)'s own convention) followed by a blank line and the rest of the body unchanged; a body with no such block is untouched. `tasqx_core::frontmatter::split` is the one parser both this and D121's memory-browser renderer (`tui/memory.rs::doc_lines`) call, replacing that screen's own inline block-finder. A store written by older code is repaired the same way on open (`storage::flatten_stored_frontmatter`, a targeted `UPDATE` per fenced row, gated on one indexed `EXISTS` so a healthy store pays nothing).

**Why:** `docs_fts` is external-content FTS5, so `memory.search`'s `snippet()` reads `docs.body` directly — a doc added with a raw frontmatter block (`memory add`'s own CLI path, and MCP's `tasqx_add_memory`, neither of which went through `memory.import`'s unrelated frontmatter cut, #228.4) put the block's `---` delimiters and its raw `key: value` lines straight into the snippet, e.g. `…How an SDK release is cut, tagged and announced --- Cut the release…`. D121(e) had already solved this exact problem for the interactive browser's OWN rendering, but "read one way everywhere" (CLAUDE.md's one-JSON-API rule, one noun over) does not hold when the fix lives in a renderer three call sites away from the write path MCP's tool and `memory show`/`memory.get` also depend on.

**Not cut:** `memory.import`'s frontmatter cut (#228.4) stays as it is — that corpus is throwaway agent-session metadata (`originSessionId`, `modified`) nobody reads as prose. A memory doc's own `description` or `author` is deliberately authored content, so flattening converts it to readable text rather than discarding it: a query matching only inside the frontmatter (its value's own words) still finds the doc, exactly as it did before this fix.

**Left standing:** the browser's dash-fence rendering in `doc_lines` is not dead code — a store `store.import` restores byte-for-byte, so a backup taken before this fix (or a hand-built import) can still land a raw fence, and that screen still reads it. `render::doc_summary`'s frontmatter-`description` shortcut (used only by the browser's own list-row preview, not any JSON result) still checks for a literal `---`; for a freshly-flattened doc it falls through to the generic first-line summary, which shows the flattened `key  value` line rather than the bare value. Cosmetic, TUI-preview-only, and not covered by an automated string-exact assertion; left as a known trade-off rather than a second special case.

**Where:** `crates/tasqx-core/src/frontmatter.rs` (new: `split`, `flatten`), `crates/tasqx-core/src/engine/memory.rs` (`memory_add`, `memory_update`, `memory_import`'s per-doc insert), `crates/tasqx-core/src/storage.rs` (`flatten_stored_frontmatter`, called from `migrate`), `crates/tasqx-cli/src/tui/memory.rs` (`doc_lines`, reusing the shared parser instead of its own).
