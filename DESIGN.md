...
| `tasqx_get_task` | R | `ref` (short_id or UUID), `annotations_limit?`, `annotations_offset?` | full task incl. deps and a page of annotations, newest first, with `annotations_total` and `annotations_next_offset` (D63) | `task.get` |
