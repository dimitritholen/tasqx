...
|---|---|---|---|---|
| `tasqx_list_tasks` | R | `filter` (DSL string), `sort?`, `limit?`, `offset?`, `fields?` | `{count, total, next_offset, tasks[]}` (D70) — the transport supplies a page when `limit` is absent; `fields` may project `depends_on` | `task.list` |
...
| `tasqx_summary` | R | `group_by`, `filter?`, `metrics[]`, `all?` | grouped counts/estimates/overdue, plus the `filter` and `all` it applied (D69) | `report.summary` |
...
| **MCP** | `tasqx mcp serve` with the §7 tools over stdio, scoped read/write per **D7**. Responses are bounded by the transport rather than by the caller's luck: `task.get` pages its history and drops the duplicate block (D63, D66, D72), `task.list` pages its rows and reports what it withheld (D70). |
...
**The add path reads the task back.** `task.add`'s result is a frozen five-field summary (D56) that cannot carry tags, due, priority or estimate, and unfreezing it to feed a renderer would move the API for a paint job. So the interactive path issues a follow-up `task.get` — the same composite shape `pick` (list + start) and `modify` (its follow-up `tag.add`) already use — and a failed read-back falls back to the plain line rather than an error, because the add succeeded and the echo failing must not turn that into a red exit. Nothing about the API moved.
...
**What D76 keeps:** everything but the `show` layout. The `add` card, the `card.*` roles and their achromatic ruling, the `caps.unicode` stdout-only gate, and the byte-identical plain rendering for every pipe, script and docs example are all untouched — the plain path still prints the exact old layout, and the parity guard still walks its label column and demands every spelling verbatim in the card, over the same toggled-off fixture matrix. The rail reuses the existing semantic roles (`danger`, `warn`, `timer.active`, `accent`, `muted`) rather than minting new ones, so a user theme that already themes those themes the rail.
...
**Not done here, left open:** #143's own suggested direction named three more surfaces still printing raw ISO instants and durations — the `list`/`agenda` DUE column and the `done`/`add` lifecycle echoes. Those live in `task_table`, `done` and `task_added_card`, not `detail_rows`, a different seam from this cluster's scope ("task-detail rendering convergence"); #143 stays open for that remainder rather than being closed by this entry.
