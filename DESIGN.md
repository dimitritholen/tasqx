...
### D145 — A closed task has no unmet blockers: `blocked` is derived from `unmet_blockers`, one query, so the two fields cannot disagree (extends D11)

**Decision:** `unmet_blockers` gates on both ends of the edge — the blocker must be unresolved (D11: not `done`, not `cancelled`) *and* the dependent itself must be open — and `is_blocked` is defined as `unmet_blockers` being non-empty. `task.get` computes `blocked` and `unmet_blockers` from that one query. The bulk set the `task.list` snapshot builds carries the same two-sided gate as one statement over every task, because a per-task query per row is the D70 cost this snapshot exists to avoid.

**Why.** `blocked` had carried the dependent's own status since the day a task completed while its blocker was still open kept answering `blocked: true` forever, and a fresh `dependency.add` could set the flag on a closed task after the fact: *blocked* is meaningless for work that is finished. `unmet_blockers` was added later (finding #8, audit-2026-09) as "the rows `is_blocked` counts", and copied the blocker-side gate only. The two then disagreed on exactly that case — a done dependent answered `blocked: false` beside a non-empty `unmet_blockers`. The CLI never showed it because the detail and `why` renderers return early on `blocked: false`; `unmet_blockers` is a frozen published field (the conformance suite, D56) so every API and MCP consumer saw the contradiction. The September 2026 test audit found it by deleting the terminal gate from `unmet_blockers` and watching the whole workspace stay green: the function had no behavioural test at all (task #69).

**Why derive rather than mirror.** Three copies of one rule was the defect's cause. Two of them are per-task and now one function; the third is a set-valued statement whose shape is different for a reason and whose comment names the query it must agree with. A rule that exists in one place cannot drift from itself.
