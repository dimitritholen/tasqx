...
### D89 — `task.modify`'s result gains `set`, and `task.add`'s gains `scheduled` — the D69 gap on the two most-called writes closed (additive per D56, tasqx audit #174)
