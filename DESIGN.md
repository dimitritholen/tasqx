...
| **Check** (**D138**, ruled; not built) | `id`, `task_id`, `body`, `state` (`open`\|`passed`\|`failed`), `evidence?`, `position`, `created`, `modified` | An acceptance criterion with a state and a citation. `evidence` is stored verbatim and never interpreted; tasqx executes nothing. Carried by `store.export`/`store.import` like every other child row (D12, D37). |

The task row gains one nullable field under **D139** (ruled; not built): `budget_tokens`, a size gauge compared against `input + output + cache_creation` and never against cache reads, with no counterpart for time because `estimate` is the time budget. Neither addition appears in the canonical export form below, which is the shipped shape.
...
Ruled and not yet built, each with its own entry in §12: `task.brief` (**D136**), `report.outcomes` (**D137**), `check.add` / `check.set` / `check.remove` (**D138**). All three rulings are additive under the frozen `"tasqx":"1"` API — new methods and new params on `task.done`, no existing response field renamed or removed (D56). D139 adds no method at all: `budget_tokens` is a field on `task.add` / `task.modify` and a pair of derived keys on `task.get`.
...
**Scheduled, ruled, and not in the roster above.** Four §12 entries add to this surface and none of them has been built; the table stays the shipped set, because the guide and the README derive their rosters from the running server and a row here that the server does not serve is a roster with two truths. `tasqx_brief_task` (R, `task.brief`, **D136**) answers "what do I need before starting this" in one call — the task, its dependency neighbourhood with the prerequisite's newest annotation, and memory hits under a query tasqx derives rather than the caller guesses. `tasqx_outcomes` (R, `report.outcomes`, **D137**) reports rework, calibration, cost, silent completions and abandonment over the event log, each rate beside its own `n`. `tasqx_add_check` / `tasqx_set_check` / `tasqx_remove_check` (W, `check.*`, **D138**) carry acceptance criteria as stateful rows with an `evidence` citation tasqx stores and never interprets, and `tasqx_complete_task` grows `checks_passed[]` and `evidence?`. Under **D139** `tasqx_add_task` and `tasqx_modify_task` grow `budget_tokens`, and the task reads report `fresh_tokens` and `over` beside it. Both of the new read tools are on the **read** scope on purpose, for the reason `tasqx_search_memory` is: an agent that cannot write should still be able to orient itself and to see its own record.
...
| **Time tracking** | `start`/`stop` intervals give real est-vs-actual data that powers burndown and throughput for free. D6's single-clock auto-stop is scoped to the caller under **D140** (ruled, not built): starting against a timer another session holds is refused rather than silently stopping it. | `task.start` / `task.stop` |
...
| **Outcome reporting** (**D137**) | tasqx measures what work cost and never whether it worked. Rework (`done` then `reopen`), estimate calibration, silent completions and abandoned effort are all already in the event log; reading them is what makes any efficiency claim falsifiable instead of asserted. Ruled, not built. | `report.outcomes` (new read, no new writes) |
| **Task brief** (**D136**) | The documented agent loop spends four to six round-trips before the first edit, one of them a *guessed* memory query that fails silently when the terms are wrong. One read returns the task, its dependency neighbourhood with the prerequisite's outcome, and memory hits under a query derived from the task itself. Ruled, not built. | `task.brief` (new read over `task.get` + `memory.search`) |
| **Acceptance checks** (**D138**) | Criteria live in annotation prose today, so nothing can ask at completion time whether they were met. A check is a claim with a citation — stateful, exported, never executed by tasqx — and completing with one open is counted by D137 rather than refused. Ruled, not built. | `check.add`/`set`/`remove`, `task.done {checks_passed, evidence}` |
| **Token budgets** (**D139**) | Token accounting is retrospective only. `budget_tokens` makes spend a live gauge over fresh tokens, and an overrun is the first evidence this document has ever carried about what "agent-sized" means. Stops nothing by design. Ruled, not built. | `task.add`/`task.modify {budget_tokens}`, derived `fresh_tokens` |
...
>
> **Not yet built:** the bare-ref command form (`tasqx 42 +blocking`, `tasqx 42 done`) and the fuzzy verb matching beside it; the rest of §5's REF grammar (ranges, comma lists, `@active`/`@last`); D8's `.is` suffix and saved-filter expansion (`@overdue`, `@today`); the socket-client daemon auto-spawn half of D5; the no-daemon OS-scheduler path and actionable toast buttons (§9b); plugins and hooks (§6); sync (D3). And the four agent-efficiency rulings, decided in §12 and scheduled: `task.brief` (**D136**), `report.outcomes` (**D137**), acceptance checks (**D138**) and token budgets (**D139**). They are one body of work and their order is decided rather than free: **D137 is built first**, because it reads history already in the store and so produces a baseline retroactively, and because every claim the other three make — fewer setup tokens, less rework, better-sized tasks — is a claim it is the instrument for. Building any of the others first would ship an improvement that could only be asserted. **D140** is ruled ahead of all four and for a different reason: it is not an improvement but a correctness fix — under parallel agents D6's auto-stop silently truncates the `tracked` column D137's calibration metric is computed from, and an instrument built over a column that is quietly wrong is worse than no instrument.
...
| **Agent efficiency** | `report.outcomes` first (**D137**), then `task.brief` (**D136**), acceptance checks (**D138**) and token budgets (**D139**). Additive throughout: new methods, new params, one nullable column and one child table; no existing response field moves, so the D56 freeze holds and the conformance suite gains rows rather than changing them. |
...
### D137 — `report.outcomes` measures whether the work worked, not what it cost, and every rate it prints carries its own denominator (ruled; not built)
...
### D138 — a check is a claim with a citation, never a command tasqx runs, and an unproven completion is counted rather than refused (ruled; not built)
...
### D139 — `budget_tokens` is a size gauge over fresh tokens, it stops nothing, and its overruns are the decomposition evidence the roadmap has never had (ruled; not built)
...
### D140 — leases are deferred; what ships instead is that D6's auto-stop stops the caller's own clock and refuses somebody else's (amends D6; ruled, not built)
