...
### D168 — `task.brief` excludes its own task's annotations from memory hits, a hit carries its `project`, and `memory.import` gains a batch-level `--project` (extends D136, D147; additive per D56)

**Decision:** three changes, one for each half of #607 and #640.

1. `task.brief`'s memory half drops any hit whose `source` names the task's OWN annotation (`task:#<short_id>`) before the D147 reservation runs, and `annotations_total` drops by the same count — it still answers "how many of this kind a wider page of this query would show", now over the rows actually worth a slot. A task with a project keeps D136's scope unchanged: that project plus unscoped docs, the `include_unscoped` shape D147's own "what does not change" note already named. A task with NO project runs no project filter at all — store-wide, the same answer `memory.search` gives when no `project` is named — because there is no scope here to widen FROM, and "unscoped only" would silently answer a narrower question than the one asked.
...
3. `memory.import` gains an optional batch-level `project`, scoping every doc it lands (`tasqx memory import <dir> --project <name>`); `memory.add` and its CLI already had one (#134). Refused the same way every project-taking write is (D23): an unknown name is `not_found`, naming `tasqx init`. Omitted, a brand-new doc lands global and an existing one (a re-import) keeps whatever scope it already had, mirroring `standing`'s own re-import rule (D143); named, it MOVES an existing doc's scope on re-import, because a caller who names one has an opinion the batch is stating. `IMPORTED_DOC_ROW` gains the same additive `project`.
...
**Rejected: `exclude_task_id` as a new `memory.search` param.** It would let the brief exclude its own notes through the same call it already makes, but it is a parameter every OTHER caller of `memory.search` — a person searching, `tasqx_search_memory` — has no use for, and growing a general-purpose method a special case for one caller is the drift D30 warns against. The filter runs in `derived_memory` instead, on the one caller that has a task to exclude, over the rows `memory.search` already returned.

**What does not change.** `memory.search`'s own scopes, ranking, ordering and `total`/`has_more` semantics; `task.brief`'s reserved-docs half (D147) and its `matched`/`reserved_docs`/`docs_total` fields, computed from the SAME scoped query the annotation filter runs over — only `annotations_total` moves, and only down, by the count of the task's own notes the filter dropped. `memory.add`'s own `--project` (#134) is untouched.
