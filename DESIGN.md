...
**The first dry run missed, and the fix went into the product.** Its agent searched `SDK 3.0 release`, got `count: 0` because no single entry holds all three words, did not try a second wording, and went on without the ruling. Rewording the prompt until the agent found it, or seeding the store with an entry that happens to hold all three words, would have made the take show a search that works only in that room, which is the staging D192 forbids. The miss was a product defect, and it was fixed as one: D193's any-word fallback, merged before this take was recorded, is why the same query now returns both entries under `relaxed: true`. The prompt is the one the dry run used.
