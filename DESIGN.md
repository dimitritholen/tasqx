...
**(a) Decision — the clock is pinned from the environment, and the pin lives in the CLI.** `TASQX_NOW`, an RFC 3339 instant, is the reference instant for every date the CLI *spells*: `due tomorrow`, `2d ago`, `created today`, an agenda heading and its `+14d` horizon, a chart's axis and window, the dashboard's panels, and the reference `now` handed to the natural-language date parser, so `due:tomorrow` resolves against the pinned day too. `crates/tasqx-cli/src/clock.rs::now` is the only wall-clock read in the crate — one door, with a guard test in that same file walking every `.rs` under `src/` and failing on a `Timestamp::now` or `Zoned::now` anywhere else, test code included. An unparsable value prints one line on stderr and exits 2 rather than falling back to the clock: a silent fallback produces exactly the drift the pin was set to prevent, and the capture would look like it worked. Unset, empty and blank are all "no pin". `scripts/demo-store.py` reads the same variable for its own "today" and passes its whole environment to every `tasqx` call it makes, so the demo store's dates and the render's reference instant come from one pinned day.

**Why the pin is CLI-side, and why it must not reach the store.** The renderers are already pure (D49): `task_table`, `task_detail`, `why`, the agenda and the charts all take `now` as a parameter, and the CLI is what supplies it — so one door under all of them pins every screen at once, with no engine change, no API parameter and nothing added to a frozen result (D56). Keeping it out of the engine also keeps it out of the store: `util::now` still stamps `created`, `modified` and every event from the real clock, so a `TASQX_NOW` left exported in somebody's shell cannot backdate real work. A capture hook that rewrote data would be a worse defect than the drift it fixes, which is also why the variable is documented in `clock.rs` and `CONTRIBUTING.md` and deliberately *not* in `tasqx docs`, the wiki or the guides: it is a capture and testing hook, not a feature.

**What the pin does not hold still, named so no capture assumes otherwise.** Urgency is computed inside the engine at the engine's own clock — `storage::map_task_row` for single-row reads, the `task.list` snapshot's one-now-per-statement read in `engine/task.rs`, and the `urgency::score`/`breakdown` wrappers — so the `URG` column, the row `next` picks and the numbers in `why`'s breakdown still move with the real day under a pin; only their labels follow it. Pinned far from today the two disagree visibly: a task the table spells `due tomorrow` scores as a deadline months away. So a captured screen is reproducible in its dates, not yet in its arithmetic, and the manifest in (c) must either keep each screen's store generated relative to the pinned day *and* accept `URG` as the one field the drift job cannot compare, or the engine gets the same door — additively, read-path only, never over `util::now` — which is the follow-up this entry leaves open.

**(b) Captured output is shown as text, not as an image.** Written when #644 lands (the ANSI-to-HTML renderer).

**(c) One manifest, and the fixtures live in the tree.** Written when #644 lands; extended by #647's executed request/response pairs.
