...
**The exclusion is a test, not a sentence.** It is sound only while every MCP tool bottoms out in a method this file freezes — the wrapper may be renamed, but the *data* a host receives is `dispatch`'s result, and those are pinned. `the_mcp_tool_layer_is_out_of_scope_because_it_bottoms_out_here` asserts exactly that, so a tool that grew a response shape of its own fails loudly with the two options written into the message (route it through `dispatch`, or extend the suite and amend this decision) instead of turning the exclusion into a blind spot.
