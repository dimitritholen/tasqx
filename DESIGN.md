...
**Test-isolation note.** `default_roots` always appends the two standard locations under the real `$HOME`/`%USERPROFILE%` regardless of a `$CLAUDE_CONFIG_DIR` override, by design, so overriding the config dir alone does not stop a scan from also reading the developer's own `~/.claude`. `attribution::tests::with_isolated_roots` moves `HOME`/`USERPROFILE` too, serialised under the existing `DISCOVERY_ENV` mutex, for the tests whose window is real wall-clock "now" (a fresh in-memory `Engine`'s own `created`/completion instants) — a live session's own calls, made moments earlier against the real store, sit inside `LOCATE_SLACK_SECS` of such a window exactly as a real one would, so the timestamp bound above cannot tell them apart and the real `~/.claude` has to stay out of reach entirely. The two pure-`compute_attribution` discovery tests, whose fixture window is a fixed 2020 date no real transcript can be near, do NOT need the override: the timestamp bound protects location the same way the date already protected blind discovery.
