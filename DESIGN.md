...
### D188 — A task's token spend is what its transcript measured: a located or session-verified Claude Code transcript outranks a self-report, and every roll-up counts only the best tier present (task #816; amends D50, D167)

**Decision:** "A task's token spend is what its transcript measured. The Claude Code transcript that did the work is the session file whose own records hold the call that started or completed the task — an MCP tool_use named `*tasqx_start_timer` / `*tasqx_complete_task` whose `input.ref` equals the task's `short_id` or UUID, or a `Bash` tool_use whose command runs `tasqx start <id>` / `tasqx done <id>`. A transcript located that way, or by an explicit `transcript_path` with session match, is graded `high` and supersedes a self-report: totals count, per task, only the highest-confidence measurement present. Self-report stays the channel when no transcript is found. Rows are never rewritten; the superseded self-report stays for audit."
