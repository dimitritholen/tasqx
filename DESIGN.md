...
**Decision:** tasqx never calls ripwire, downloads it, or shells out to it. `initialize`'s `instructions` (D141) appends one line naming whether `ripwire` — a code-mapping binary agents may already have — is on this process's `PATH` (`mcp::ripwire_on_path`, a directory scan, no spawn): found gets the map-before-grep nudge, missing points the agent at `tasqx setup`. `tasqx setup` itself prints one line when ripwire is missing, pointing at ripwire's own install instructions rather than a package name or URL tasqx would have to keep current, and exits 0 either way — the check cannot fail the command.
...
### D180 — `store.export` carries the `links` table and `store.import` restores it through one id remap table (task #783; extends D12/D37/D160)
