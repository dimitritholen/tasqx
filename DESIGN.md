...
**Tasqx** is the organiser for your AI: a backlog, a memory and a brief for a coding agent, and a hyper-modern, cross-platform task manager that lives entirely in the terminal — a Taskwarrior reimagined to be faster, better-looking, deeply extensible, and AI-native (positioning: D175). It is built as a headless Rust core engine exposing one stable, versioned JSON API; every surface — CLI, TUI/GUI, MCP server, plugins, HTML reports — is a client of that single contract. The north star is not the most features, but a fast, elegant, terminal-first task manager with the best-chosen features and first-class AI support.

---

## 1. Vision & principles

Tasqx is what an organiser is for a person, for an agent: the list in the right order, the reference section, the briefing before each piece of work, the checklist that says what done means, and the expense sheet — one local file that the agent reaches over MCP and its owner reads from the shell. That same file is a terminal-native task manager for developers, sysadmins, and power users who live in the shell and want Taskwarrior's power without its friction — faster, prettier, scriptable to the bone, and built so an AI agent is a first-class user, not a bolted-on afterthought.
...
| `tasqx_search_memory` | R | `query`, `limit?`, `scope?`, `raw?`, `include_rank?` | `{count, hits[], matched, relaxed}` bm25-ranked (D41); `matched` is the expression actually run (D69); `relaxed` says a plain query with no all-words hit fell back to any word (**D193**); each hit's bm25 `rank` is present only under `include_rank: true` (**D154**) | `memory.search` |
...
| `tasqx_add_tokens` | W | `ref`, `tool?` (defaults to the handshake client), the four counts or `total_tokens`, `model?`, `idempotency_key?` | `{short_id, measurement}` — spend recorded after the fact, always `self-report` at `medium`: `source` and `confidence` are pinned by the server and refused from the caller (D167); `total_tokens` is one unsplit count, never beside the four | `token.add` |
...
|---|---|---|
| **Git-first sync** | **D3** (§12) | **Not built.** Sync is a pure *consumer* of the append-only event log, which has shipped and is written transactionally with every mutation (the load-bearing invariant). Because the log is already the record of truth, the git backend (`store.export` → commit → merge, per-field LWW) can land later without a migration, and the CRDT-per-field upgrade after that is additive on top. Deferring costs nothing structural; building it now would freeze a conflict policy against zero real-world merge evidence. The evidence is now being gathered one door down: `store.import --merge` (**D185**) applies D3's per-field LWW to a task a second live store already holds — unioning its notes, checks, tags and edges, taking each scalar field group from the side whose latest event touched it, and counting tracked time from the union of both event logs (**D189**) — so the policy, read off the event log as D3 prescribes, is exercised over the export document before any backend is committed to. |
...
| **Agent efficiency** | `report.outcomes` (**D137**), `task.brief` (**D136**), token budgets (**D139**) and acceptance checks (**D138**) have all shipped. Additive throughout: new methods, new params, one nullable column and one child table; no existing response field moves, so the D56 freeze holds and the conformance suite gains rows rather than changing them — which is what D137's own landing demonstrated. `task.brief`'s own scoping was then tightened (**D168**): it excludes the task's own annotations from its memory hits, each hit names its `project`, and `memory.import` gains a batch-level `--project` so an executor's CLI-only writes can land scoped in the first place. |
...
**Why:** Recycling is a safety bug: a stale terminal scrollback makes `tasqx done 42` hit the *wrong* task. Sparse climbing integers are a trivial cost next to that; users who truly want density opt in knowingly. (`id` UUIDv7 remains the real key regardless.) **`store.import` is the one write that moves a number without being asked, and only ever onto a fresh one: see D177, which replaces the refusal this entry's remedy once pointed at.**
...
**What does not change.** `task.stop`'s `interval` and `tracked`, D97's windowing (an adjustment, like a D98 correction, has no place in time and is not attributed to a window), and the stop event's `tracked`. (D189 later added `interval_started` to the stop event's payload, naming the interval it closed.)
...
**Decision:** a measurement carries either the four buckets or one `total_tokens`, never both — `task.done` and `token.add` refuse the mix. The column is `token_usage.total_tokens` (0 on every existing row, which is what those rows mean). It is reported as its own figure everywhere a measurement is: `total_tokens` on every measurement object, `tokens_unsplit` as a `report.summary` metric and a key of `report.outcomes`' `cost`, `total (unsplit)` in `tasqx show` and in the MCP task view. It is never divided into input and output, and no transcript or telemetry parser writes it. `token.add` leaves `UNEXPOSED_METHODS` as the write-scoped `tasqx_add_tokens`. That tool records exactly what `tasqx_complete_task`'s self-report records: `source` and `confidence` are pinned to `self-report`/`medium` (`mcp::PINNED_ARGS`) and refused if the caller sends them, and `tool` defaults to the handshake client. An agent that could pick its own grade could certify itself `high` and void the ladder below. The JSON method keeps its full params for the CLI and `tasqx api`; `tasqx tokens add <ref> --total N | --in N --out N …` records the same self-report at `medium` from a shell. `token.add` names every missing required field in one refusal. The D155 roster cap moves from 32,768 to 34,304 bytes, because a tool was added: 31 tools measured at 34,000 bytes.

**Why a separate kind.** Agent harnesses often report one number. Before this, the choice was to invent a split (and D48's cost report then prices a guess), put the whole number in `input_tokens` (and every in/out ratio lies), or record nothing. A kind that says what it is keeps the four buckets honest and still records the spend.

**Why the gauge counts it in full.** D139's gauge counts fresh tokens: input + output + cache creation, not cache reads. An unsplit total may include cache reads that nobody separated, so counting all of it can overstate fresh spend. Counting none of it, or a guessed share, would let an unsplit reporter never overrun. The gauge is a signal that stops nothing, so a false overrun costs a look and a missed one costs the signal. The budget hint says so.

**The confidence ladder, stated once.** Confidence grades how firmly the tokens are tied to this task, not how precise the counts are. `otel` is `high`: samples are matched to the task by session id. `log-parse` is `high` only when an explicit `transcript_path` was parsed and the completion's session id was confirmed in it. It is `medium` when a path was given but the file carries no per-session anchor (Gemini, Copilot), and `low` when the transcript was found by scanning and matched on time overlap alone. `tokens.recompute` downgrades a row to `low` when its transcript is gone. A self-report is `medium` by default and may be marked `low`; `high` is refused (D50). The agent is the only party that knows which task the spend served, so it is the primary channel. But nothing tasqx holds can check the number, and confidence describes verifiability, not how much the reporter is trusted. The levels are unchanged; this ruling only writes them down where an agent reads them: the two tool descriptions and the token-accounting guide.

**Rejected: raise self-report to `high`.** The finding asked whether `medium` was only "unverified". It is, and that is the whole meaning of the grade. A `high` self-report would make the grade say nothing a reader could act on.

### D168 — `task.brief` excludes its own task's annotations from memory hits inside the query, a hit carries its `project`, and `memory.import` gains a batch-level `--project` (extends D136, D147; additive per D56)

**Decision:** three changes, one for each half of #607 and #640.

1. `task.brief`'s memory half excludes the task's OWN annotations (`source = 'task:#<short_id>'`) from the annotation arm's MATCH itself — `Engine::memory_search` delegates to a `pub(crate)` `memory_search_excluding(p, exclude_task_id)`, an internal widening no public param exposes, which adds `AND a.task_id <> :exclude_task` to the arm before `LIMIT` and `COUNT(*)` both run. `annotations_total` is therefore exact, not a page-level subtraction: a task whose own notes bm25-outrank a matching sibling's ruling no longer fills the SQL-level page with notes destined to be discarded, crowding the sibling off it (review finding on the first cut, which filtered the already-limited page and could answer fewer hits than `memory_limit` — even zero — while `total` still claimed a match existed). A task with a project keeps D136's scope unchanged: that project plus unscoped docs, the `include_unscoped` shape D147's own "what does not change" note already named. A task with NO project runs no project filter at all — store-wide, the same answer `memory.search` gives when no `project` is named — because there is no scope here to widen FROM, and "unscoped only" would silently answer a narrower question than the one asked.
2. Every `memory.search` hit — and so every `task.brief` hit, which runs the same engine call — carries a `project` field, additive on the frozen `MEMORY_HIT_ROW`: null for global knowledge, a doc's own column or an annotation's task's, the same split `source` already draws. A store-wide search used to leave a reader unable to tell a hit's project without opening it, which is how #607's cross-project noise went unnoticed in the first place.
3. `memory.import` gains an optional batch-level `project`, scoping every doc it lands (`tasqx memory import <dir> --project <name>`); `memory.add` and its CLI already had one (#134). Refused the same way every project-taking write is (D23): an unknown name is `not_found`, naming `tasqx init`. One upsert statement binds it directly (NULL when omitted) for a brand-new row and reads `project = COALESCE(excluded.project, docs.project)` on conflict: omitted, a re-import keeps whatever scope the doc already had, mirroring `standing`'s own re-import rule (D143); named, it MOVES an existing doc's scope, because a caller who names one has an opinion the batch is stating. `IMPORTED_DOC_ROW` gains the same additive `project`.

**Why:** #607 and #640 hit the same seam from opposite ends. #607 found `task.brief` on ledger task #586 echoing the task's own description back as a "hit" and burying it among cross-project docs it gave no way to tell apart. #640 found the CLI-only path — an executor subagent with no MCP tools — had no way to land its own memory writes scoped at all: `memory.add` scoped one doc at a time, but a whole imported ADR directory could not, and it vanished from every `project`-scoped list the moment nobody remembered to `tasqx_update_memory` it back in by hand. The repo's own demo store reproduced the self-echo half exactly: `tasqx brief 51` returned three of five hits as `task:#51`'s own annotations, verbatim, because `memory.search` has never had a notion of "the caller's own task" to exclude.

**Rejected: `exclude_task` as a public `memory.search` param.** It would let the brief exclude its own notes through the same call it already makes, but it is a parameter every OTHER caller of `memory.search` — a person searching, `tasqx_search_memory` — has no use for, and growing a general-purpose method's frozen params list for one caller's shape is the drift D30 warns against. `memory_search_excluding` stays `pub(crate)`, reachable only from `derived_memory`. **Rejected (review): filtering the already-fetched page instead of the query.** The first cut did exactly this — `retain` over the hits `memory.search` returned, with `annotations_total` reduced by the same count — and it reads clean, but it is wrong whenever the excluded rows outnumber the slack between the page and `LIMIT`: the SQL-level cut has already happened by the time the filter runs, so a sibling's ruling ranked just past where the task's own (better-ranked) notes filled the page never reaches the filter to be kept. The exclusion has to be part of the query the `LIMIT` and the `COUNT(*)` both read, the same lesson D147 itself is named for.

**What does not change.** `memory.search`'s own scopes, ranking, ordering and `total`/`has_more` semantics; `task.brief`'s reserved-docs half (D147) and its `matched`/`reserved_docs`/`docs_total` fields — only `annotations_total` moves, now computed from a MATCH that never counted the task's own notes at all. `memory.add`'s own `--project` (#134) is untouched.
...
### D170 — a recurrence spawn carries the description and the checks, and records `spawned_from` (task #660, finding #628; extends D2)

**Decision:** completing a recurring task spawns the next occurrence with more than its fields. In the same transaction as the completion, the spawn gets (1) the predecessor's OLDEST live annotation, verbatim, as a new note with its own id and `created`; (2) every check, in the same order, with `state: open` and no evidence; (3) `spawned_from`, a nullable `tasks` column holding the predecessor's uuid, added by the idempotent migration. `task.get` returns `spawned_from` as the predecessor's short_id, the way `depends_on` and `blocks` name tasks, and null on any other task or when the predecessor is not in the store. The card's and the detail view's Repeats row read `<rule> from #N`. `task.brief` on a spawn adds `last_time: {short_id, title, completed, delivered}`, where `delivered` is the first paragraph of the predecessor's delivery note under D165's rule (the pinned note, else on a closed task the newest live note), and every brief renderer prints it first as "Last time". `store.export` carries `spawned_from` by uuid, only on a spawn, and `store.import` accepts it.

**Why:** a ledger task (#604 -> #625) lost its description on its first recurrence, and would have lost its acceptance criteria. The next occurrence is the same work again, so what describes the work and what proves it done belong on it. Later notes are the history of the occurrence that closed, and stay there; the predecessor's last word is one link away through `spawned_from`, and the brief quotes it.

**The copies write no events of their own.** The spawn's `add` event stays the one event the spawn writes, and its payload is unchanged. `undo` already refuses both `done` and `add`, so a copy cannot be taken back while its spawn stays behind.

**Rejected: `delivered` read from the newest note.** A note written on the predecessor after it closed is not what it delivered; D165's pin is, and the brief reads the same note the card's Delivered row does.

### D171 — a filtered `store.export` scopes `projects`/`docs`/`events` to what its tasks need, and reports what it dropped; an unfiltered export still carries everything (task #661, finding #627; narrows D37/D41's "always all of them")

**Decision:** `store.export {"filter": ...}` no longer ships every project, every memory doc and the whole event log regardless of `filter` — a filter that names a project (or, through its selected tasks, several) now scopes those three to what the exported `tasks` actually need. The **needed project set** is the union of every `project:`/`proj:` value the filter itself names (D109's `Filter::project_names`) and every `project` the SELECTED tasks carry — so `store.export {"filter": "project:ledger"}` carries `ledger`'s project row even when a further term (`status:done`) happens to match zero of its tasks, and `store.export {"filter": "+bug"}`, which names no project at all, still carries only the projects the matching tasks actually belong to rather than leaking every other project's knowledge. A memory doc is kept when its own `project` is in that set; an unscoped doc (`project: null`) is kept only with the new `include_unscoped: true` param (CLI `--include-unscoped`), the same widening `memory.search`'s flag of that name already does, and for the same reason `memory.search` refuses it: `include_unscoped` on an UNFILTERED export is refused (`bad_request`) because there is no scope there to widen from. An event is kept when its `entity_id` names a task, doc or project this document actually carries (`events.entity` is `task`/`project`/`doc`, D30's closed `Entity` enum) — the same trim `depends_on` already got for edges (#176). The header gains three ADDITIVE counters beside the existing `dropped_dependencies`: `dropped_projects`, `dropped_docs`, `dropped_events`. `default_project` is named only when it is among the exported project rows — a filtered export whose scope drops the store's own default used to hand back a document `store.import` refused outright, naming a default `projects` did not carry (review finding). **An unfiltered export is unchanged**: `Filter::is_unfiltered()` (true only for the empty string, `Pred::Always`) turns the whole scoping pass off, so D12's byte-identical round trip and D37's "archived included, regardless of filter" both still hold — a project or doc no task references is still carried, because there the store, not the task set, is what a caller asked to see.

**Why.** `tasqx export project:ledger` filtered `tasks` (21) but still shipped all 41 memory docs from every project, all six project rows and the full 213 KB event log — a data leak the moment the file is shared, and a surprise on import elsewhere (a "ledger" export handing a reader every other project's notes back). D37 and D41 decided ALWAYS-ALL specifically because a filter selects tasks and a project/doc a filter excludes is not a dangling pointer the way an edge is — true, and still true for the unfiltered case; what changed is that D171 treats "the tasks I exported" as the scope a filtered caller actually wants, the same "only what it needs" argument #176 already made for events.

**Rejected: scope by `filter.project_names()` alone, ignoring the selected tasks' own `project`.** A tag filter spanning two projects (`+bug`) names no project at all, so this would answer either "keep nothing" (which contradicts a task genuinely in scope still needing its project row/docs) or "keep everything" (the exact leak being closed). **Rejected: derive the scope from selected tasks alone, ignoring `filter.project_names()`.** An explicitly named but currently zero-match filter (`project:ledger status:done` when nothing is done) would then silently drop `ledger`'s own project row, contradicting what the caller named. The union is the only answer that keeps both promises.

### D172 — a tag is stored lowercased and may not contain whitespace; a filter's tag term is lowercased too; an older store is folded on open and the fold is logged per task (task #662, fixes #620)

**Decision:** `storage::normalize_tag` is the one spelling a tag is stored under: Unicode `to_lowercase`, and any whitespace refused `bad_request` naming the tag and suggesting the hyphenated form. `ensure_tag_link`, which every door that attaches a tag goes through (`tag.add`, `task.add`, `store.import`, a recurring spawn, `undo` of `tag.remove`), normalises before it looks up or inserts, so no door can store another spelling. The doors that also record or answer with the names (`tag.add`, `tag.remove`, `task.add`, `store.import`) normalise their `tags` param up front with `normalize_tags`, which also collapses duplicates, so the event payload, the response and `tag.remove`'s lookup all use the stored name and an all-or-nothing call refuses before it writes. `undo` normalises a pre-D172 `tag.remove` payload before re-attaching. The CLI's `tag`/`untag` argument list goes through the same function, so its echo marks the tag that was actually stored. The filter DSL stays an exact match, on the normalised form: `filter::predicate` lowercases the value of `+`/`-`, so `+Perf` selects `perf`. A quoted spaced tag still parses and now matches nothing, because no stored tag can hold a space.

The schema migration (`storage::normalize_stored_tags`, run on every open) folds an older store: each tag is lowercased and each run of whitespace becomes one hyphen. A name that lands on an existing tag is merged: its `task_tags` links are re-pointed with `INSERT OR IGNORE` (a task that carried both keeps one link), then its row is deleted. It runs under `BEGIN IMMEDIATE` and re-reads the list under the lock, so a daemon and a one-shot opening the same old store do not both apply it. Each task it touched gets its `rev` bumped and one `tag.normalize` event listing every `{from, to}`. `undo` refuses that op by name, because a merge cannot be split again.

**Why.** Tags were case-sensitive exact strings that could hold spaces, so `+UPPER` and `+upper` were two tags that looked like one, and `+has space` needed shell and filter quoting that agents got wrong. Normalising in `ensure_tag_link` and not in each caller is what makes "every door" true for doors added later. Refusing whitespace instead of hyphenating it on write is deliberate: a write that silently renames what the caller typed is D33's unfalsifiable answer. The migration has to hyphenate, because refusing a stored tag would leave a store nobody can open.

**No separate dry-run command.** The ruling asked for a dry run listing what would change. tasqx has no repair-reporting verb (no `doctor`), the migration runs inside `storage::open` before any command could ask it anything, and the other open-time repairs (`repair_stale_default_project`, the dependency-FK rebuild) are silent. So the report is the event log instead: `event.list {ref}` on a task shows the `tag.normalize` event with the exact renames and merges applied to it, and nothing about a merge is lost from the record. A new verb just to preview a fold that runs once per store would be surface with one use.
...
**Why (b).** `throughput`/`heatmap`/`burndown` could not be scoped at all before this — `run_chart` always called `burndown_members(engine, &None)` for the first two, so `tasqx chart throughput` answered for the whole store even inside a project-scoped review. A `--project` flag existed on `burndown` alone, one command with its own quoting convention while the other three read `project:<name>` off a shared positional — two ways to say the same thing, and only one of them composed with a second predicate. Giving all three the filter DSL fixes the missing scope on all three at once; `--project` stays as the flag's own shorthand rather than being removed, because a script or a doc already spelling `chart burndown --project x` is not this ticket's to break — the ruling asked for the filter DSL to reach `chart`, not for the flag to go.
...
### D175 — tasqx is positioned as "the organiser for your AI"; "task manager" is the category word kept in body text, never again the headline

**Decision:** the headline on every public surface is "The organiser for your AI", followed by one line that lands the mechanics: a backlog, a memory and a brief for your coding agent; one binary, one SQLite file on your disk, no account, no cloud. The surfaces are the README, the docs-site overview (`docs.rs`), `tasqx --help`'s about line, `tasqx manual`'s opening, the MCP server's instructions intro (`mcp.rs`, which says "this workspace's organiser" because an agent reads it, not a buyer), both crate descriptions, the Homebrew and Scoop descriptions (rendered by `scripts/brew-formula.sh` and `scripts/scoop-manifest.sh`, and the copies in the tap and the bucket), the wiki's two entry pages and the agent starter prompt. The README leads with what an organiser holds, one section per feature — the list, the order, the reference section, the briefing, the checklist and the expense sheet, the review — and the shell story ("For you, at the shell") is the second act, carrying the `why` sample the guard in `tests/readme.rs` pins. "Task manager" stays in body text, the wiki and the manual so a search for one still lands here; it is not the headline anywhere.

**Why.** "Terminal task manager" names a shelf the reader already has something on, and nobody installs a second. The features that are only tasqx's are all on the agent side — `brief` and what a prerequisite concluded (D147, D165), memory over imported docs and annotations (D41), checks with evidence (D138), token cost (D50), `--outcomes` — and the old headline pointed at the half that is table stakes. "Organiser" rather than "memory": memory is one of six things it holds, and the nearest neighbour already sells itself as a memory upgrade for a coding agent, so the word that separates the two is the one that names the whole set. The subhead exists because the bare phrase can be read as a tool that tidies AI chats or files, which a web search shows are real products; "a backlog, a memory and a brief" closes that reading in the next line.

**Rejected: "your AI's chief of staff".** It carries the accountability half (checks, outcomes, cost) better, but it is already another product's line and reads corporate. **Rejected: dropping the shell story.** The shell is where the owner reads the file and where the pictures come from; it is demoted to the second act, not removed. **Rejected: a Homebrew description starting with "The".** `brew audit` refuses a description that opens with an article, so the package string is the article-free "Organiser for your AI: …" on both package managers, one string in both render scripts.

### D176 — the README opens on a hand-drawn SVG map of what the organiser holds; the VHS recording is no longer the first picture (amends D163; follows D175)

**Decision:** `docs/img/hero.svg` is the first picture in the README: one node per part of the organiser D175 names — the list, the order, the briefing, the expense sheet, the checklist, the reference section, the review — each with a one-line explanation, the store in the middle, and the two clients, the shell and the MCP server, feeding it. It is plain SVG written by hand: no script, no web font (an `<img>` cannot load one, so it names the system monospace stack and every line is kept short enough for the widest fallback), and its own dark background, so it reads the same on GitHub's light and dark themes and beside the terminal captures below it. D163's recording, `docs/img/hero.gif` with `scripts/hero.tape`, stays in the tree as the capture of `add`, `dep` and `pick` that `docs/maintainers/terminal-style.md` lists, but nothing links it from the README.

**Why.** D163 chose a recording because a still could not show capture, "what now?" and the overview as one sequence. D175 moved the pitch: the first screen now has to say what tasqx holds for an agent, and that is a map, not a sequence. Nodes with a sentence each are read in the time the GIF took to loop once, and they are the same list the README's sections then walk. SVG rather than PNG because the text stays crisp at any width and a wording change is a diff, not a re-render.

**Rejected: deleting the GIF and its tape.** D163 deleted the still it replaced, but that still could be re-rendered from a fixture. The recording is the one capture outside D149's loop, and the other tapes point at `hero.tape` for their requirements, so it stays until a later ruling finds it a place or drops the tapes together.

### D177 — `store.import` renumbers a task whose `short_id` another task holds, keeps the stored number for a known id, and reports every move (task #781; amends D4's import remedy)

**Decision:** a payload task whose `short_id` a DIFFERENT task in this store already holds keeps its `id` and takes the next free number, minted from the ordinary counter; the import no longer refuses. Every move is reported as `renumbered: [{id, from, to}]` in `store.import`'s result — always present, empty when nothing moved, the same rule `projects_created` follows — and `tasqx import` prints one line per move naming both numbers and the id. The counter is raised above EVERY `short_id` in the payload before the first task is written, so a minted number can never collide with a row the same document still has to write and no cascade renumbers a task that had no conflict. A task whose `id` this store ALREADY holds is not renumbered and not reported: the upsert's `ON CONFLICT(id)` path no longer writes `short_id`, so the stored number stands whatever the document says, and only a task new to this store is checked against the number at all. Two payload tasks claiming one number is still a `conflict` — that is an incoherent document, not a collision — and so is a payload task at a lower `_rev` than the stored one (#177). Uniqueness of `short_id` within the document is checked over the whole payload before any row is written (#796), so the refusal never depends on row order or on what the destination holds.

**Why.** D4's refusal named a remedy nobody had: "import into a fresh store, or renumber", with no renumber anywhere in the product and a fresh store defeating the purpose of the import. Exporting from one machine and restoring into another machine's live store hit it on the first task, because both stores mint from 1. The number is a display handle, not the key: `short_id` is a foreign key nowhere — dependencies, annotations, checks, links and events all key on the UUIDv7 `id` — so moving it costs exactly what a human wrote down, and the task itself, its history and its graph arrive intact. The stored-number rule is what makes the operation idempotent: a second import of the same document is a no-op, rather than walking the task's number up the counter on every run. D4's substance is untouched — a number is still never recycled, and a minted one still only climbs.

**Rejected: a `--strict` flag that keeps the refusal.** Nobody asked for one, and the case it would protect — a restore into a fresh store — never collides, so the default behaviour there is unchanged. A flag would be a second contract to document and test for a payload shape that already cannot hit this path. **Rejected: renumbering every task in the payload onto a fresh block.** It is simpler to reason about and loses every number that was free, which for an import into an empty-ish store is all of them; the collision is usually a handful of rows, and the ones that fit keep the numbers their source used. **Rejected: doing this as `gc --renumber`.** D4's compaction is a different operation — a loud, confirmed, whole-store densification the owner asks for — and it cannot help a caller who is holding a document their store refuses.

### D178 — `initialize` and `tasqx setup` say whether `ripwire` is on PATH; tasqx never calls it (§10 no phone-home)

**Decision:** tasqx never calls ripwire, downloads it, or shells out to it. `initialize`'s `instructions` (D141) appends one line naming whether `ripwire` — a code-mapping binary agents may already have — is on this process's `PATH` (`mcp::ripwire_on_path`, a directory scan, no spawn): found gets the map-before-grep nudge, missing points the agent at `tasqx setup`. `tasqx setup` itself prints one line when ripwire is missing, naming the upstream repository URL (https://github.com/redhat-et/ripwire) rather than a package name tasqx would have to keep current — still running no download — and exits 0 either way — the check cannot fail the command. (amended by #795: ripwire ships no Homebrew formula and no Scoop manifest tasqx could depend on or suggest, so the hint names the upstream repository URL instead of pointing at "its own install instructions".)

**Why.** An agent that reaches for ripwire only when it is already there is a hint, not a dependency; fetching or invoking a binary tasqx does not ship is the phone-home line §10 already draws, and a `PATH` lookup is the whole cost of telling the agent the difference.

### D179 — `memory import` stores a doc's source relative to the git toplevel, so one file is one doc from any directory and any machine (task #784; extends D174)

**Decision:** `crates/tasqx-cli`'s `memory_docs_from_path` no longer stores `file.display()` as given. A new `import_source(file)` canonicalises the file, then walks up from its directory for a `.git` entry — file or directory, since a worktree's is a file — and, if found, stores the path relative to that toplevel, `/`-joined regardless of platform. Outside a git work tree, it falls back to the path relative to the canonicalised current directory; if the file is not under that either (an `../elsewhere` import), it stores the canonicalised absolute path. The engine's `source` column and its D174 uniqueness rule are untouched — this only changes what the CLI writes into it. After the import call, `run_memory_import` makes one unlimited `memory.list` call and, for every doc whose `source` is non-empty and not one of the sources this batch just wrote, checks it as a path suffix against each new source (`/`-normalised, either ending in `/` + the other) — `./docs/a.md` and an absolute `/home/x/repo/docs/a.md` both end in `/docs/a.md`; `guides/a.md` does not — and prints a `note:` line naming it as a possible older spelling, reported under a `superseded: [{id, source, matches}]` key in the JSON result. Nothing is deleted or rewritten. #797: because `import_source` canonicalises, a directory holding a real file and a symlink alias of it computes the same `source` for both, which `memory.import`'s D174 uniqueness would otherwise refuse outright — `memory_docs_from_path` dedupes by that computed source first, keeping a non-symlink entry as the winner regardless of sort order (`std::fs::symlink_metadata`, not the directory's sorted-first name, #798) and naming every actual symlink path in a `note:` line and, under `--json`, an `aliases_skipped: [{source, via}]` key, always present — and the `memory.list` scan for older spellings runs after the import already committed, so a failure there (a closed backend) is reported as one `note:` line rather than turning a successful import into a failed command. `slash_joined`, the one line all three `import_source` branches share, only replaces `\` with `/` under `#[cfg(windows)]`; a unix filename may legally contain a literal backslash, and replacing it there used to fold `docs/a\b.md` onto an unrelated `docs/a/b.md` (#798).

**Why:** D174 made `source` a doc's identity, so `docs/`, `./docs/` and `/Users/me/repo/docs/` on another machine used to be three docs for one file — the reason CLAUDE.md had to tell every session to spell an import path identically, and the reason a docs folder imported once locally and once from CI never matched. A git toplevel is the one anchor two checkouts of the same repo share; the current directory is the next-best anchor when there is no repo. Reporting the mismatch rather than silently carrying it forward lets a caller find and retire the stale doc instead of it quietly drifting out of search once nobody imports its old spelling again. A path-suffix compare, not a bare file-name compare, is what keeps `docs/a.md` from flagging an unrelated `guides/a.md`.

**Rejected: a migration that rewrites existing sources.** The old spelling on disk is not recoverable from the stored string alone — `docs/a.md` does not say whether it used to be `./docs/a.md` or an absolute path on a different machine — and D174 already refuses to delete a doc from a migration; this decision's `note:` line does what a migration could not: point a person at the pair and let them decide. **Rejected: shelling out to `git rev-parse --show-toplevel`.** A subprocess for what the stdlib's own directory walk answers is an extra dependency on `git` being installed and on its process-spawn cost, for a search `std::fs` already does. **Rejected: normalising `source` in the engine.** The engine has no notion of a current directory or a git repository — `source` is an opaque string to it, by design — so the normalisation belongs at the one door that reads a filesystem path in the first place, the CLI's importer.

### D180 — an imported doc remembers its origin file: absolute path, mtime and size, set only by `memory.import` and never identity (task #788; extends D174, D179)

**Decision:** `docs` gains three nullable columns — `origin_path` (the file's absolute path on the importing machine), `origin_mtime` (its modification time in unix seconds) and `origin_size` (its size in bytes) — added by the same additive `add_column_if_missing` pattern `project`, `rev` and `standing` used, with NO default, so every doc a store already holds reads back with all three null. `memory.import` is the only writer: each batch entry may carry them, they are validated like any other optional param (a wrong type is a `bad_request` naming the key), and they sit in the upsert's SET list plainly — the opposite of `standing`/`project`'s COALESCE rule (D143), because they describe THIS read of the file, so a re-import that carries none leaves the row saying it has no origin rather than keeping a path from an import on another machine. `memory.add` and `memory.update` never set them. `memory.get` and `store.export` state all three, null when absent, and `store.import` carries them back, so a backup round-trips. The CLI's `memory_docs_from_path` fills them from `std::fs::canonicalize` and `std::fs::metadata`, omitting a field the platform cannot answer for rather than sending a zero. `source` is untouched: it remains a doc's identity and the key a re-import replaces by (D174), spelled relative to the git toplevel (D179).

**Why:** nothing on a doc row said which file it came from or what that file looked like, so tasqx could not tell a doc that still matches its file from one whose file has been edited since — the question the refresh sweep (#789) and the stale flag (#790) both have to answer, and neither can answer it by re-reading every byte of every file on every turn. A path, a size and an mtime are what a filesystem answers for free. They are deliberately NOT identity: two machines canonicalise the same repo file to different absolute paths, which is exactly the drift D179 removed from `source`, so `origin_path` is a pointer back at one machine's filesystem and nothing keys on it.

### D181 — `store.export` carries the `links` table and `store.import` restores it through one id remap table (task #783; extends D12/D37/D160)

**Decision:** the export document gains a `links` array and a `dropped_links` counter, and `store.import` gains a `links` section and a `links_imported` count — both always present, the rule every other counter in these two results already follows. The row is `link.list`'s own, read through the same `link_row`/`LINK_COLS`, so the archive and every reader spell a link the same way: `{id, from, to, relation, metadata, created_at}`, the ends as `<type>:<uuid>`. `created_by` is deliberately not in it, for the reason `LINK_COLS` already states — nothing reads the column and `'user'` is the only value anything writes; the day a machine-proposed edge exists, the field joins the row. EVERY export, filtered or not, keeps a link only when BOTH ends are nodes this document carries — a task in the exported set, a doc in `docs`, a project in `projects`, or an annotation belonging to an exported task, which is all four kinds the `links` table admits — and counts the rest, the same trim `depends_on` has had since D12. An unfiltered export carries every task, doc and project row, so its `dropped_links` is 0 and D12's byte-identical round trip now covers the graph's edges too — with one exception the scope exists for: `annotation.remove` tombstones the row (D113) and the export deliberately carries no tombstone, so a link to a REMOVED annotation is dropped and counted rather than exported as an end the import would then refuse, which is what used to make a store with one such link unable to back itself up at all. Link EVENTS stop travelling whole: `Some(Entity::Link)` is now scoped to the links this document carries, exactly like the task, doc and project kinds beside it — which means a REMOVED link's `link.add`/`link.remove` pair no longer travels, on a filtered export or an unfiltered one, because D171's rule is "the event names something this document carries" and a deleted row is not carried. That is the same history a removed memory doc's events already lose, and the trim is counted in `dropped_events` as it is for every other kind. A removed annotation is not a node for a link end either — `graph::node_exists`'s annotation arm requires `removed IS NULL`, so `link.add` and `store.import`'s own existence check both refuse an edge to a tombstone the same way they refuse one to an id nobody holds (task #799).

On import, links are written in a pass AFTER tasks (with their annotations), docs and projects, because a link spans all four and the endpoint columns are polymorphic, so there is no `FOREIGN KEY` to defer to. Each end is resolved in three steps: the `<type>:<uuid>` grammar, then `remap: HashMap<(NodeType, String), String>` — payload id to the id that row actually landed under, filled by the passes that write those rows — then existence. An end the store does not hold after all three refuses the whole import with `bad_request`, naming the link, the end's type and its id, in the shape D12's dangling-`depends_on` refusal already has. The remap holds the kinds whose stored id CAN differ from the document's: a project, because `upsert_project` keys on NAME and a destination that already knows that name keeps its own row and id; and a doc, which is where #782's source-merge will land one. A kind or an id the table does not name resolves as itself, which is what every task and annotation does — D177 moves a task's `short_id`, never its id. The row is upserted `ON CONFLICT(id) DO UPDATE`, never `INSERT OR REPLACE` (the annotations upsert states why). A payload link that duplicates an edge this store already holds under a DIFFERENT id leaves the stored row alone and counts as imported: `(from, to, relation)` is the table's other UNIQUE, which an `ON CONFLICT(id)` upsert cannot see, and the same edge stated twice is the idempotent answer `link.add` already gives as `created: false` — and that pairing is recorded in a second, link-only remap (payload link id to the stored row), which the payload's link EVENTS pass through before they are written, so the imported history names the row this store kept instead of a link nobody here holds. Those events are the one section held back out of order for it: the links pass cannot move above the events pass, because a link's ends are rows that pass has not written, so the link events are staged as they are read and inserted after it. The relation passes the same closed-vocabulary gate `link.add` enforces, `metadata` the same object-or-nothing rule, `created_at` the same date gate every other imported stamp passes, and the resolved ends the same self-link refusal — import is not a back door around an invariant the API enforces at its own door. An id repeated inside one document's `links` array is a `conflict`, #180's rule for `tasks` at this door too: the upsert would apply both and keep only the last while `links_imported` counted two. A document with no `links` key imports exactly as before, at `links_imported: 0`.

**Why.** `store_export` only ever passed link events through ("Links are not scoped by this export yet"), so a full backup restored into a store with no graph edges at all, beside a replayed log describing every edge that was supposed to be there — the state D12 was written to make unreachable for dependencies, reached one table over. The remap exists before it is needed because the alternative is a links pass that hard-codes "the payload's id is the stored id": true for a task today and already false for a project, whose row the destination may own under another id, and about to be false for a doc.

**Rejected: reconstructing links from link events on import.** The log is history, not state; D12 already restores state from rows and history from events separately, and a store whose `link.remove` events were trimmed (or whose log a filtered export scoped) would rebuild a different graph than it exported. **Rejected: exporting links unscoped on a filtered export.** An end the document does not carry is exactly the dangling pointer the import's own refusal is there to catch, so an unscoped filtered export would produce a document that cannot be restored anywhere — the failure D12 named for `depends_on`. **Rejected: an `import` event per restored link.** Tasks and projects get one because their rows are the state; a link's history already travels in the `events` section and is replayed there, so a second marker would only be another bookkeeping row the next export has to exclude.

### D182 — Memory freshness is checked on demand against the origin file, never watched; source code is mapped live by an external tool, never indexed in tasqx (task #792; extends D41, D74, D178, D179, D180)

**Decision:** two rulings, recorded together because they are the same principle answered on either side of one boundary — what tasqx stores versus what it defers to a tool that already does the job live. On memory: an imported doc's `origin_path`, `origin_mtime` and `origin_size` are recorded once, at `memory.import` (D180). Checking freshness never re-reads a file on its own initiative; it happens on demand, at three doors — `tasqx memory import --refresh`, a write-scoped MCP `initialize`, and every hit `memory.search` or `task.brief` returns — and each does the same two-step check: mtime and size against what the filesystem answers now is the fast filter, and only a mismatch opens the file for the truth, an exact compare of the freshly read title and body against what is stored, so a `touch` with no byte changed is not reported as one. `--refresh` and `initialize` re-read and land what changed, bump `rev`, and name what they could not find under `missing` without deleting it — the file may be off this machine, not gone from the world. A hit instead carries `stale: true`, told and left for the reader to act on, because a search is not the door that writes.

Source code gets the opposite answer: tasqx does not index it. A code mapper already on the operator's `PATH` — ripwire, D178 — answers where a symbol is defined, who calls it and what a change would touch, live and per invocation. A symbol index kept inside tasqx would be a second copy of that answer, staler the moment either tool runs again, and its symbol-name traffic would win ranking slots under D41's lexical BM25 that a stored ruling should have held.

**Why:** a stat call per imported doc at a session's `initialize` and per hit a search or brief actually returns costs milliseconds and is exact, because the filesystem already answers it for free — the same fact D180's decision leaned on. A watcher or a daemon timer would run for the whole life of a process to save that one stat, at the moment D74 is retiring the daemon tasqx already has; `notify` is not a dependency this project carries or is adding one for. The same shape answers the code question: ripwire's own incremental cache is the live index, kept warm by the tool that owns the language grammars, and tasqx re-deriving a weaker copy of it buys staleness, not currency.

**Rejected: importing a code map as memory docs.** It goes stale the moment either tasqx or the mapper runs again, on the store's own clock rather than the file's — the exact drift `origin_path` exists to detect for prose, and a symbol dump has no comparable "unchanged" fast path. **Rejected: a brief-time shell-out to the mapper from tasqx.** It couples a product surface to a third-party binary's presence and exit code; the agent already runs the mapper itself in the time a stat takes. **Rejected: a path-aware memory search** ("what touched `engine/memory.rs`"). FTS5 already tokenises a path's segments, so a plain query finds it; a second parameter for what the existing one already answers is drift D30 would name.

### D183 — `store.import` merges a memory doc onto the destination doc that already holds its `source`, later `modified` wins, and reports every merge (task #782; extends D174, twins D177)

**Decision:** a payload doc whose non-empty `source` a DIFFERENT doc in this store already holds is that doc, not a second holder of its source: the stored row's id is KEPT, the payload's id is DROPPED, and the import no longer refuses. The lookup — `SELECT id, modified FROM docs WHERE source = ?1 AND source <> ''` — runs before the `_rev` guard and before D174's `refuse_source_held_elsewhere`, because neither one is about this case: the counters of two independently minted rows say nothing to each other, and the stored holder IS this doc. `modified` decides the text, the one field that means the same thing on both machines: strictly later than the stored stamp and the kept row takes the payload's `title`, `body`, `search_body`, `project`, `standing` and `modified`, with its OWN `rev` bumped by one (D143's rule for a text-changing write, never the payload's counter) → `took: "payload"`; equal or older and the row is not touched at all → `took: "store"`. `origin_path`/`origin_mtime`/`origin_size` are left alone either way — D180 says they describe one machine's read of one file, and this row was read on the other machine. Every merge is reported as `docs_merged: [{source, kept_id, dropped_id, took}]` in `store.import`'s result — always present, empty when nothing merged, the rule `renumbered` and `projects_created` already follow — and `tasqx import` prints one `note:` line per merge naming the source, both ids and the copy that won. `dropped_id` is null when the payload doc named no `id` of its own, rather than the id this pass mints for it, so `dry_run` (D184) and a real run report the same merge instead of two different minted ids (#801). The doc counts in `docs_imported`, and writes the same `memory.add` event every imported doc writes, under the kept id. D181's `remap` carries the rest: `(NodeType::Memory, dropped_id) -> kept_id` replaces the identity entry the docs pass used to insert, so a payload link naming the dropped id lands on the survivor, and a payload doc EVENT naming it is re-pointed the same way — no staging needed, unlike D181's link events, because the docs pass already runs above the events pass. A merge is reported even when nothing was written (`took: "store"`), because the report is about the id the import dropped, not about the bytes it changed; that is what makes the second import of one document report the merge again while leaving the store byte-identical. A payload doc with no `source`, and one the store already holds under the SAME id, are untouched by all of it — the second still goes through #84's rewind guard. #800: a payload `docs` array naming one non-empty source twice is refused whole, `bad_request`, before any doc is written — the same fault `memory.import` already refuses for its own array (D174) — and a payload doc whose OWN id is already a different live doc here (the lookup above matches by `source`, not by that id) is refused as `conflict` rather than merged, because two identities for one file cannot fold into one row. The staged link events D181 holds back for pass 2c also name their ends inside the event's own `payload` JSON, not only through `entity_id`, so a `from`/`to` string naming a doc this merge dropped is rewritten to the kept id there too.

**Why:** D179 spelled `source` relative to the git toplevel so one file is one doc on any machine, and D174 made that string the doc's identity. What neither could fix is the id: `memory.import` mints a fresh UUIDv7 per store, so two machines that each ran `tasqx memory import docs/` over the same repo hold the same docs under the same sources and different ids. Importing one store's export into the other then tripped `refuse_source_held_elsewhere` on the first doc, with `conflict` and a remedy — "update that doc instead" — that no import can carry out. That is exactly D177's defect one table over, and it has D177's answer: the collision is on a key the destination owns, the arriving row keeps everything that matters, and the move is reported rather than refused. Merging cross-machine memory is the normal case for a tool whose whole point is one agent's knowledge following its owner between machines; refusing it made the backup format unable to do the one thing a second machine needs from it.

**Rejected: deterministic doc ids derived from the `source`.** It removes the collision at the root, and it needs a migration that rewrites every existing doc's primary key — every link, every event and every annotation reference following it — and leaves the store with two identity schemes, the derived id and the UUIDv7 rows minted before it, for as long as any old store exists. **Rejected: keeping today's refusal.** It is the state that made cross-machine memory unmergeable, and its message names a remedy the caller holding a document cannot perform. **Rejected: `_rev` as the tiebreak.** The two counters were minted independently, on stores that never saw each other's writes, so a payload at `_rev` 99 is not "ahead of" a stored row at 3 — it is a different count of a different history. `modified` is the only stamp both machines mean the same thing by. **Rejected: keeping BOTH rows and clearing the loser's `source`,** the remedy D174's own migration uses for pre-existing duplicates. That migration could not consult a second copy of the text; an import can, and leaving a sourceless twin behind would put the same ruling in the search index twice, once under a key nothing will ever refresh again.

### D184 — `store.import` takes `dry_run`: the whole import runs and rolls back, so `tasqx import --dry-run` previews renumbering, merges and refusals against a live store (task #785; extends D12)

**Decision:** `store.import` gains a `dry_run` boolean param, default false. When true, every pass runs exactly as it does today — renumbering (D177), doc merges (D181/D183), link resolution, the same refusals at the same points — inside the same `BEGIN IMMEDIATE` transaction, and the only thing that changes is the last line: `tx.rollback()` in place of `tx.commit()`. The answer is built from the same in-memory counters either way, so `renumbered`, `docs_merged`, `links_imported` and every other field describe exactly what a real import would do; `dry_run` itself is added to the answer, always present, `false` on a real run, the same rule every other additive result field here already follows. A refusal surfaces identically on either path — it is still a `bad_request`/`conflict` returned before the transaction reaches its end, dry or not — and no event survives a rollback, so `event.list` sees nothing and a second connection sees nothing, because both only ever saw the transaction after it closed. `tasqx import <file> --dry-run` sends `dry_run: true` and nothing else changes about the call; the human echo prefixes its first line with `dry run:` and ends with one line, `nothing was written`.

**Why:** the cross-machine merge D183 shipped turns `store.import` into the one write in this backlog that used to be irreversible and unrehearsable at once — `tasqx undo` does not reach into an import's transaction, and a merge decided which doc's text won before the caller could see it decide. Running the whole thing and throwing the transaction away is the same trick `tokens.recompute`'s `dry_run` and `memory.refresh`'s already use one layer up: SQLite's own rollback is the exact inverse of every write inside it, for free, so a preview costs nothing and drifts from the real path never — there is no second copy of the rules to keep in sync as D177/D181/D183 evolve.

**Rejected: a planning pass that predicts the outcome without writing.** It is a second implementation of every rule in `store_import` — the renumber-above-every-`short_id` step, the merge's `modified` comparison, the link remap, every refusal's ordering — read-only and shaped just differently enough from the real pass that the two would drift the first time either one changed alone. Run-then-rollback has exactly one code path. **Rejected: a `--yes` prompt before committing.** The CLI is scripted (`tasqx export | tasqx import -` and CI restores both assume no TTY), so a prompt is either skipped by a flag that reintroduces the irreversibility it was meant to guard, or it blocks every non-interactive caller; `--dry-run` first and a second, real invocation is the shape a script can express without inventing a new one.

### D185 — `store.import` takes `merge`: a known task's notes, checks, tags and edges are unioned and its scalars follow the later `modified`, instead of the payload replacing them wholesale (task #786; amends D138 for the merge case, applies D3 at the import door)

**Decision:** `store.import` gains a `merge` boolean param, default false. Default is today's behaviour, unchanged: a task whose `id` this store already holds has its annotations, checks, token measurements, tags and dependency edges DELETEd and reinserted from the payload, because the payload's task object is authoritative about its own child rows (D138), and a payload behind the stored `_rev` is refused (#177). `merge: true` changes exactly one case — a task whose `id` ALREADY exists here — and changes nothing at all for a task new to this store, which goes through the ordinary path, renumbering (D177) included. For a known task: (1) **annotations, checks and token measurements are unioned** — nothing is deleted, and each payload row is upserted under its OWN id, the `ON CONFLICT(id) DO UPDATE` the annotation door already used; (2) **tags and dependency edges are unioned** — the destination's stand and the payload's join them, and the self-dependency and cycle guards run on the union, not on the payload alone; (3) **the `_rev` guard is skipped** for that task; (4) **the scalars** — title, status, priority, project, due, scheduled, wait, estimate, recurrence, remind, completed, `active_since`, the tracked total and its adjustment, `budget_tokens`, `spawned_from`, `delivered_annotation_id`, every column the upsert's SET list carries — are taken from the payload only when the payload's `modified` is STRICTLY later than the stored `modified`; otherwise the upsert does not run at all and the stored row stands beside the child unions. `rev` becomes `max(stored, payload) + 1` when anything changed, so neither store's `expected_rev` guard (D13) reads the merge as a rewind; "anything changed" is the scalars having moved, or the union having added a child row or an edge — counted from the rows themselves, because an upsert reports a row affected even when it rewrote the same bytes, which is what keeps merging one document twice a no-op rather than a walk up the counter. A check is the one child row that resolves a same-id conflict on a stamp of its OWN (task #802): it carries state, so a criterion both stores hold takes the payload's body, state, evidence and position only when the payload check's `modified` is strictly later than the stored one — a stored `passed` is not rolled back to `open` by a copy that never saw it pass, and a criterion edited here is not thrown away because the payload's TASK happened to be the later write — and that update counts as a change for the `rev` bump, since counting child rows can only see an insert. That comparison reads `checks[].created`/`modified` through the same `opt_when` date gate the task's own timestamps pass rather than raw, so a malformed stamp is refused by name instead of parsing as `None` and reading as older than everything the strict-later test compares it against, which used to skip a payload check's body, state, evidence and position without saying why (task #803). An annotation carries only `created`, so it has no such stamp and a note both stores hold under one id follows the TASK-level comparison instead: the stored body stands when the store won it, the payload's when the payload did, and an id this task does not hold is inserted either way. An annotation id and a check id are global primary keys, so one already held by a DIFFERENT task in this store is refused `bad_request` naming both tasks rather than silently moved — one annotation, and one check, belongs to exactly one task, which is the rule `tokens[].id` has enforced at its own door since it was written. That refusal is decided once, in the same pre-loop pass that raises the `short_id` mint floor, against the store as it stood before any task in the payload was written, rather than per task inside the child-row loop: decided there, the answer depended on payload order, since the non-merge path had by then already deleted an earlier task's own children and freed an id a later task went on to claim (task #803). And the merge's skip of a measurement the task already holds applies exactly once: an id the PAYLOAD itself repeats is the ordinary duplicate-id refusal, because one measurement id standing for two rows of spend cannot be applied at all. Every merged task is reported as `merged: [{id, took}]`, `took` being `"payload"` or `"store"` — always present, empty when `merge` is false or the document carried nothing this store had seen, the rule `renumbered`, `projects_created` and `docs_merged` already follow — and `tasqx import <file> --merge` prints one `note: merged: task <id> took the <payload|store> copy` line per entry. `merge` composes with `dry_run` (D184): `--merge --dry-run` runs the whole merge and rolls it back, so which side wins each task is readable before anything is kept.

**Amended by D189.** Point (4) is narrowed: the scalars no longer move as one unit on the task-level `modified`. Each field group follows the side whose latest event touched it, the tracked pair is counted from the union of both event logs, and `merged` names the fields. The task-level comparison survives as the fallback for a group neither log touched, for a copy with no log, and for a note both stores hold under one id.

**Why:** D177 and D183 made two live stores importable into each other, and this is the third and last place the same defect sat. `store.import` was written for a restore — one document, one empty-ish store — so a known task's child tables were replaced wholesale and a payload at a lower `_rev` was refused outright. Between two machines that is not a restore: a note written on B since A's export vanished the moment A's copy carried the higher `_rev`, and the other direction was simply refused, so neither machine could fold the other in without losing what it had written. Annotations, checks, tags and edges are append-mostly sets that commute — §3 and D3 both already say so — so a union is the answer that loses nothing, and the only genuinely conflicting fields are the scalars, where D3's per-field last-writer-wins already names the policy. `modified` is the one stamp two independently minted stores mean the same thing by, which is exactly the argument D183 made for a doc's text. This is D3's LWW arriving at the import door, without the sync server D3 puts in a later phase: the door already holds every validation, every graph invariant and every refusal, so nothing had to be restated to reach it.

**Rejected: making the union the default.** A restore would then resurrect a note the owner deleted, a tag they removed and an edge they cut, because a backup document has no way to say "this row is gone" — a tombstone format D113 deliberately keeps out of the export. It would also change D138's contract silently, for every caller who is restoring rather than merging, which is most of them. **Rejected: a `_rev` comparison across stores.** The two counters were minted independently, on stores that never saw each other's writes, so a payload at `_rev` 99 is not "ahead of" a stored row at 3 — it is a different count of a different history. That is D183's reasoning for a doc, and it is the reason the guard is skipped rather than re-aimed. **Rejected: a sync server.** D3 already schedules one for a later phase and says why git-based comes first; a merge that runs at the import door needs no infrastructure, no process to operate and no second copy of the rules, and it works over the document format the product already ships.

### D186 — `tasqx_start_timer` gains a `memory` block through `include_memory` (default true), and `tasqx_list_tasks`'s `@working` page and `tasqx_add_task` gain a tiny standing-rulings addendum (task #97; extends D136, D147, D157)

**Decision:** `tasqx_start_timer`'s MCP envelope gains `include_memory`, a `TRANSPORT_ONLY_ARGS` boolean beside `task.start`'s `keep`/`actor`, default **true**. When true the response carries `memory` — exactly `task.brief`'s own memory half, computed by the new `pub(crate) Engine::task_start_memory` (engine/task.rs), which resolves the same `ref` and calls the same private `derived_memory` `task_brief` already calls, so the two tools search identically and never drift apart. The page defaults to **three** hits (`mcp::START_MEMORY_LIMIT`), smaller than the brief's five (D154), because a caller of this tool reads no task half beside it — the whole answer has to fit as a bonus on `task.start`'s own small result rather than as a read of its own. A hit carries no `rank`: this tool advertises no `include_rank` of its own, so the D154 float is dropped unconditionally rather than gated behind an opt-in nobody can reach. `include_memory: false` answers `task.start`'s own frozen result exactly, byte for byte — the JSON API and `tasqx api task.start` never see the argument at all (it is not in `dispatch::PARAMS`), and the conformance suite's MCP case for this tool now asks for `include_memory: false`, mirroring what D151 and D152 already did for their own tools, so it still checks the frozen result rather than the new default one.

`tasqx_list_tasks` and `tasqx_add_task` each gain a `rulings` field — a JSON array of doc titles, three at most (`mcp::TASK_RULINGS_TITLES`) — never a new argument, so neither tool's schema or `PARAMS` moves. `tasqx_list_tasks` carries it only when the caller's `filter` contains the literal `@working` keyword — the filter `TRACK`'s own instructions text already tells an agent to pick work with — and scopes it to the SESSION's own project, `Engine::session_rulings`'s own inference (workdir ancestor, else the default project, D157), the same project `initialize`'s rulings section already answers for; a filter that does not ask to pick work carries no addendum, and neither does a session with no project inferred. `tasqx_add_task` carries it for the project the new task actually landed in (`task.add`'s own `project` field, D21) rather than the session's, because a task can be filed under a project the caller is not standing in. Either way the titles are the project's standing docs if it has any (`memory.list {project, standing: true, limit: 3}`), else its newest (`memory.list {project, limit: 3}` — `session_rulings`'s own "standing if there is one, else newest" rule, run through the existing `memory.list` method rather than a second SQL statement). The block is omitted entirely, not sent as `rulings: []`, whenever there is nothing to say — a project with no memory docs at all, or `project: null`.

Measured 2026-09-23 on a scratch store carrying two standing docs, three ordinary notes and two tasks in one project (`tasqx mcp serve`, `tools/call`, JSON block bytes): `tasqx_start_timer` with `include_memory: false` answered 252 bytes, the same call with the block on (three hits, a mix of docs and the one matching annotation there was room for) answered 992 bytes — **+740 bytes** for the memory half. `tasqx_list_tasks {filter: "project:tasqx"}` answered 394 bytes; the same page under `{filter: "@working"}` answered 483 bytes, **+89 bytes** for a two-title `rulings` array; `tasqx_add_task` under the same store answered 277 bytes carrying the identical two-title block. The D155 roster cap moved from 34,816 to 35,072 for `include_memory`'s schema property, the only argument either tool's roster entry grew by (`tasqx_list_tasks`/`tasqx_add_task` took no new argument, so their entries did not move).

**Why.** #95/D147 already put rulings ahead of a task's own notes inside `task.brief`'s memory half, on transcript evidence that a project's own chatter can bury the correction a session most needs. #97's own transcript evidence is one step earlier: across 18 sessions, `tasqx_start_timer` was called in every one of them and `tasqx_brief_task` in only 7, with the first edit landing before the first memory read in 13. An agent that skips the brief and goes straight to starting the clock — which the evidence says is the common path, not the exception — never saw a ruling that `task.brief` would have shown it. Moving the memory read onto the call that is actually made, rather than telling agents to make a different call, is the same shape of fix D136 itself was: the tool that is used answers the question, instead of a workflow document asking a session to remember an extra round-trip. The two smaller additions extend the same idea to the other two calls a session makes before `task.brief` is ever in the picture — picking work and filing new work — at a page size (three titles, no bodies) that a call already carrying a page of tasks or a single new one can afford to answer without troubling its own budget.

**Rejected: a `memory_limit` argument on `tasqx_start_timer`, mirroring `task.brief`'s.** `task.brief`'s caller reads a memory page on its own, so a caller-chosen size is worth exposing; `tasqx_start_timer`'s caller is starting a clock, and three hits already cost 740 of the 992 bytes the call now spends — a second knob for a value nobody asked to change in #97's own evidence is the unrequested control D30 already warns against. **Rejected: reusing `derived_memory` directly instead of a new `pub(crate)` wrapper.** `derived_memory` and the `resolve_ref`/`task_tags` calls that feed it are private to `engine::task`, on purpose — a private helper composed from other private helpers, not a method with its own contract — and `mcp.rs` is a sibling module, not a descendant of it. Raising `derived_memory` itself to `pub(crate)` would have exposed a second entry point with the SAME signature `task_brief` already calls it with, inviting a caller-supplied `Task`/tags pair to drift from what `resolve_ref` would have resolved; the one-line `task_start_memory` wrapper is the smaller surface, callable only with a `ref` the same way `task_brief` is. **Rejected: reusing `session_rulings` for `tasqx_add_task`'s addendum too.** `session_rulings` answers for the WORKDIR's project, which is right for `tasqx_list_tasks`'s `@working` page (picking work happens where the session is), but a task can be explicitly filed under a project the session is not standing in — `session_rulings` would then show the wrong project's rulings beside a task that named a different one by hand. The `memory.list`-backed `rulings_titles` helper takes an explicit project either caller supplies, so `tasqx_add_task` scopes to the task's own and `tasqx_list_tasks` scopes to the session's, from the one function.

### D187 — A behaviour baseline widens the docs-fixtures harness to the whole user-visible surface, in a second manifest a de-bloat PR diffs against (task #740; extends D149)

**Decision:** `crates/tasqx-cli/docs-fixtures/baseline/` is a second fixture directory with its own `manifest.tsv`, captured and checked by the SAME `scripts/docs-capture.sh` D149(c) already runs against `../manifest.tsv`, now taking a `--dir=PATH` flag that swaps both the manifest and the output directory it reads and writes. It is a second directory rather than more rows in the existing one because `fixtures.rs`'s `screens!` macro `include_str!`s every name `../manifest.tsv` lists, held equal to it in both directions by a test (D149(c)) — a row added there ships inside `tasqx docs` whether or not it belongs on the guide, and this corpus is explicitly the surface a de-bloat diff checks, not a page a reader browses. 122 rows capture what the site's 69 leave dark: every top-level verb's and subcommand's `--help` (73 rows — the flag inventory, and so the feature inventory), `--json` for every read verb whose JSON shape no `api-*` row already covers (13), `--card`/`--card --ascii` for the four verbs that take it (5), a new `pipe-plain` `kind` docs-capture.sh now understands — the render path a script actually gets piping tasqx with `TASQX_FORCE_COLOR`/`TERM` unset, which every row in EITHER manifest had forced to the coloured terminal layout until now (10 rows, proven different: `list-plain` reads `- 1 overdue -` and `*` for the running task where the coloured `list` reads `·`/SGR/`▶`), `tasqx docs --stdout`'s one page (which is `cli_ref`/`api_ref`/`mcp_ref`/`obj_ref`/the filter grammar/the config keys all at once, static, needing no store), an `initialize`-then-`tools/list` handshake piped into `mcp serve` at both scopes (`%b` now expands a manifest stdin field's `\n` into real newlines, since a JSON-RPC handshake is more than one line and a TSV field cannot hold one), and default-mode captures for every verb neither manifest showed before (`export`/`memory add` excluded — see below).

**Why a second `capture_pipe` render mode rather than a maintainer setting `TASQX_FORCE_COLOR`/`TERM` in the manifest row itself.** The row format has no per-row environment column, and adding one for a single flag would be more surface than the one boolean `pipe`/`pipe-plain` already is; the two are near-duplicate bodies in `capture_pipe` rather than one body threaded through an env array, because an empty `"${arr[@]}"` under `set -u` is an unbound-variable error on bash 3.2 — `/usr/bin/env bash` on a maintainer's Mac, not only on CI's.

**What was tried and excluded, found by the corpus itself.** `export` and `memory add` were captured, then dropped, when this task's own determinism proof (capture twice, diff) caught both: the demo store's one RUNNING task is left running by a live `tasqx start` inside `demo-store.py`, which mints a real event id at the pin instant, and `export`'s full-store dump quotes that id's random tail the same way D149(c) already excludes `task.add`'s; `memory add`'s echo quotes the fresh doc UUID it just minted, with no short-id form the way `add-echo`'s task id has. `config set`/`unset`/`theme set` are absent because they write `config.toml`, which — unlike `tasks.db` — this harness does not copy per row, so a mutating config row would make every later row depend on manifest order; `config path`/`config store` are absent because they print an absolute `TASQX_CONFIG_DIR`/`TASQX_DB` path, this machine's, the same reason D149(c) drops `about`. `daemon` and `watch` join `dashboard`/`pick`/the memory browser's interactive siblings as uncapturable: a daemon holds a socket open and `watch` streams until killed, so neither is a pipe or a tui-with-a-`sleep`. None of this is a gap in the mechanism — it is D149(c)'s own three exclusion classes (a fresh entropy-pool id, a machine path, shared mutable state) recurring one layer further into the surface, which is exactly what a second sweep over the same mechanism should find.

**Why not build a second harness.** #740's brief is explicit about this, and the corpus bears it out: every new row is a `scripts/docs-capture.sh` row, rendered by the same `capture_pipe`/`capture_tui`, against the same pinned demo store, diffed by the same byte-for-byte `--check`. The only code changed is `docs-capture.sh` itself (`--dir`, `pipe-plain`, `%b`), on the same script every future widening reads.

**Rejected: folding the baseline into `../manifest.tsv` behind a `#[cfg(not(feature = "docs-site"))]`-style filter in `fixtures.rs`.** `fixtures.rs`'s equality test is the whole safety net that keeps the site from silently going stale (D149(c)); teaching it to skip some rows for embedding purposes reopens exactly the drift it exists to close, for a distinction — "captured for the guide" vs. "captured to prove nothing broke" — that a second small file states for free. **Rejected: a `--json`/`--card`/`plain` row for every verb, exhaustively.** A mutating verb's `--json` is the same bytes `dispatch::PARAMS` already returns to `api-*`'s JSON-envelope rows, since both paths render the one JSON API (`CONTRIBUTING.md`'s "one JSON API" rule); duplicating that shape under a CLI flag would multiply rows without adding a byte the corpus does not already watch. The 13 `--json` rows are exactly the read-shaped, human-table verbs no `api-*` row reaches.

**Addendum (PR #145 review): CI is a second machine, and it found a fourth exclusion class the same afternoon it went required.** `setup-list` and the two `mcp-initialize-*` rows passed locally and failed on ubuntu, on bytes neither this entry's determinism proof nor its bite proof had touched, because both only ever ran on one machine. `ripwire_on_path` (D178) — read by `tasqx setup`'s hint line and by every MCP `initialize`'s own `instructions` — scans `$PATH`, and a maintainer's Mac carrying `ripwire` answers a different sentence than a CI runner that does not. `session_rulings`'s workdir-ancestor project inference (D157) reads the process's own working directory, and GitHub Actions checks this repo out under `/home/runner/work/tasqx/tasqx`, whose ancestor `home` IS one of the demo store's own project names — so `initialize` answered with that project's standing rulings on ubuntu and fell through to the store's default project on a Mac, whose checkout path matches none by coincidence. Both are the same shape as `export`/`memory add` above — something a captured row reads that is not the pinned store — but neither is inherently machine-specific the way a fresh v7 id or an absolute path is, so exclusion was not the answer: a fourth `docs-capture.sh` `kind`, `pipe-isolated`, runs the row from `$work` (this run's own `mktemp -d`, a sibling of `/home` rather than a descendant of it) with `HOME`/`PATH` pointed at paths nothing has ever installed into, so every machine now answers "nothing installed" and infers the same default project. Proved by running `--dir=...baseline --check` under `env -i` with a freshly minted `HOME` and a `PATH` built from scratch, excluding the directory `ripwire` actually lives in on the machine that found the bug.

### D188 — A task's token spend is what its transcript measured: a located or session-verified Claude Code transcript outranks a self-report, and every roll-up drops the self-report it superseded (task #816; amends D50, D167)

**Decision:** "A task's token spend is what its transcript measured. The Claude Code transcript that did the work is the session file whose own records hold the call that started or completed the task — an MCP tool_use named `*tasqx_start_timer` / `*tasqx_complete_task` whose `input.ref` equals the task's `short_id` or UUID, or a `Bash` tool_use whose command runs `tasqx start <id>` / `tasqx done <id>`. A transcript located that way, or by an explicit `transcript_path` with session match, is graded `high` and supersedes a self-report: when a task carries any `high` measurement whose source is not `self-report`, its self-report rows drop out of every total; everything else sums. Self-report stays the channel when no transcript is found. Rows are never rewritten; the superseded self-report stays for audit."

**Correction (review, same task): a ref match alone is not evidence.** The first cut located a transcript on `input.ref` matching alone, with no time bound — any real call for that ref, anywhere in the tool's whole history, located the file. Proved wrong by this repository's own `~/.claude`, which holds a real `tasqx_complete_task` call for ref `"1"` from an unrelated session: a `TASQX_DB=<scratch> tasqx --no-daemon done 1` run from a dev build would have been "located" as that real task's transcript. Two fixes, both at the matching rule rather than in test isolation: (a) the call's own line `timestamp` must fall within `LOCATE_SLACK_SECS` (ten minutes — hook latency is milliseconds, but a bound has to exist at all) of the window edge it names, a start call near `window_start` and a done call near `window_end`; (b) a `Bash` command that sets `TASQX_DB=` before the `tasqx` word it runs — within the same simple command, split on `&&`, `||`, `;`, `|`, `&` and newlines, the program word being exactly `tasqx` or `*/tasqx` with no `=` — targets a scratch store (the dev-build shape `CONTRIBUTING.md` requires) and never matches, so a developer's own throwaway run is never read as the real daemon completing a real task.

**Why now.** Every `token_usage` row in the live store carried `source=self-report` — 27 of 36 with `output_tokens: 0`, cache read/write zero everywhere — while the real Claude Code transcript for one of those sessions measured `input: ~400, output: ~105k, cache_read: ~43M`. D50's log-parse path never lands on a self-reported task because `compute_attribution` returns before ever looking at a transcript (the old `attribution.rs:460` early return), and agents almost never pass `session_id`/`transcript_path` on completion (35 of 881 completion events did, and those carried an MCP bridge id — `session_01…` — that never matches a Claude Code transcript filename). The only reliable anchor left is the one thing every Claude Code transcript already contains: the record of the call that started or completed the task.

1. **Dedupe, `tokens::claude_code::parse_samples`.** A sample's primary key is `(message.id, requestId)`, keep-last as before; a line with no `requestId` keys on `message.id` alone (the pre-existing behaviour, unchanged). A fallback key of `(message.id, timestamp)` also dedupes a sidechain replay — the same `message.id` re-emitted under a NEW `requestId` but the same timestamp — into the same sample, the shape ccusage's own parser treats as one duplicate. A new `tokens::claude_code::contains_task_call(path, short_id, uuid, window_start, window_end)` answers whether a transcript's own records name the task AND time-bound the call to the window it names (the correction above), and `tokens::claude_code::session_files(main)` names a session's full file set: the main `<session>.jsonl` plus every `<session>/subagents/*.jsonl` beside it.
2. **Location, `attribution::locate_transcripts_by_task_call`.** With no explicit `transcript_path`, before falling back to today's window-overlap discovery (`discover_samples`, still `low` confidence, still one-shot terminal, still capped at `MAX_DISCOVERY_FILES` newest by mtime), the daemon scans the Claude Code roots for a `*.jsonl` `contains_task_call` matches. Candidates come from `locate_candidates`, NOT `discover_candidates`'s newest-N cap: a subagent file is excluded at candidate-selection itself (never treated as a session of its own, only ever read as part of the main file `session_files` reaches from it), and the bound is `mtime >= window_start` rather than recency rank — the #817 backfill re-attributes tasks completed days or weeks ago, and a handful of busy days of subagent files (which used to count against the recency cap before being filtered out) could otherwise push any older task's session out before `contains_task_call` ever read it. If the start and done calls landed in different session files (a `/clear` in between), both files' samples are used. Only Claude Code carries this evidence today; every other parser gets an empty result and keeps today's rules. **Correction (PR #149 review): the walk filters as it goes.** The shared `walk` stopped at `MAX_DISCOVERY_FILES * 8` files counted in `read_dir` order before `locate_candidates` filtered them, so on a real `~/.claude` with thousands of old or subagent files a session was dropped by read order alone; location's walk now applies the subagent and `mtime >= window_start` filters while walking, and only eligible files count toward its large safety cap `MAX_LOCATE_FILES`. Discovery's walk is unchanged. **And a located file is a shared source (D50).** Two concurrently active tasks in one session carry no `transcript_path` or `session_id`, so the done-payload join never saw them share anything and both banked the samples inside both windows. `PendingAttribution::located_neighbours` carries every other task whose window overlaps and that shares no known source; once a transcript is located, a neighbour whose own call `contains_task_call` finds in the same file, or whose explicit path is that file, contests like any D50 foreign window. Residual: a task with an EXPLICIT path does not look for located neighbours in its file (the recompute mirrors that branch and has no location step); a located neighbour that banks later is still refused the explicit task's claimed sample ids store-wide.
3. **The self-report early return narrows.** `compute_attribution`'s `pa.self_reported` early return now fires only when NOTHING was located; when a transcript is located, log-parse still runs and grades `high` — the same grade an explicit path with a verified session id already earned, and for the same reason: the call itself is the correlation. `engine::tokens`'s two D50 mixing guards narrow to match: `token_attribute`'s `self_reported_meanwhile` TOCTOU suppression exempts a `high`-confidence write, and `token_add`'s self-report-after-attribution refusal exempts an existing `high`-confidence row. Below `high`, both guards are unchanged — a low- or medium-confidence automated measurement still cannot be joined by, or joined to, a self-report, because that measurement really would have nothing above it and would be silently doubled either direction.
4. **Aggregation, one helper.** `engine::tokens::rollup_measurements(measurements)` takes one task's rows and applies the one rule: if any row is `high` and its source is not `self-report`, the task's `self-report` rows are dropped; every other row sums. It is what `measurement_totals` (the `task.list`/`-tokens` field and sort key, and `task.get`'s `fresh_tokens` budget-hint gauge) and `report.summary`/`report.outcomes` (D139's per-task gauge and its cost buckets) sum through, replacing the raw `SUM` in `fresh_tokens` and the direct `for m in &snapshot.tokens` loops the other three used. A task carrying a self-report AND the transcript that superseded it therefore counts once, at the transcript's numbers, everywhere a total is read — while `task.get`'s own `tokens` list (`Engine::tokens_of`) stays the full, unfiltered audit trail D56 already promises: no row disappears, only the roll-up drops one. **Correction (PR #149 review): the first cut kept only the top confidence tier**, which also dropped rows that are separate spend — two self-reports (a `medium` and a `low`) used to sum and stopped summing, and a reopened task's `high` located cycle erased its `low` discovery cycle. Only a self-report is ever superseded, so only a self-report is ever dropped.

**Why supersede rather than replace.** A self-report already committed at `task.done` time is a fact about what the agent claimed; the located transcript is a fact about what actually happened. Keeping both, and letting aggregation pick the higher-confidence one, is D50's own append-only discipline (§7 "no hidden bulk delete", the same reasoning `tokens.recompute` already follows for a log-parse row) applied to a case that discipline had not reached: the self-report was never wrong to record, and it stays checkable against the transcript that later confirmed or corrected it.

**Why the confidence reuse rather than a new grade.** `confidence_for(transcript_parsed, session_correlated)` already answers `high` for `(true, true)`; a located transcript is graded by passing that same pair, because the two cases share what actually earns `high` — a specific transcript is now KNOWN to belong to this task's call, not merely inside its time window. A fourth grade for "found by its own call" would fork the ladder D167 already wrote down for one reason (how the samples were found) that this is another instance of, not a new one.

**Rejected: also matching a Bash-run `ref` against every neighbour word.** `bash_command_targets` requires `tasqx start`/`tasqx done` immediately followed by the exact ref as its own whitespace-delimited word, not a substring — ref `4` must not match a command naming task `42`. A looser match would let a session's mention of one task falsely locate a transcript for a different one.

**Test-isolation note.** `default_roots` always appends the two standard locations under the real `$HOME`/`%USERPROFILE%` regardless of a `$CLAUDE_CONFIG_DIR` override, by design, so overriding the config dir alone does not stop a scan from also reading the developer's own `~/.claude`. `attribution::tests::with_isolated_roots` moves `HOME`/`USERPROFILE` too, serialised under the `DISCOVERY_ENV` mutex (now `crate::tokens::DISCOVERY_ENV`, so the Copilot roots test reading `HOME` takes it too), for the tests whose window is real wall-clock "now" (a fresh in-memory `Engine`'s own `created`/completion instants) — a live session's own calls, made moments earlier against the real store, sit inside `LOCATE_SLACK_SECS` of such a window exactly as a real one would, so the timestamp bound above cannot tell them apart and the real `~/.claude` has to stay out of reach entirely. The two pure-`compute_attribution` discovery tests, whose fixture window is a fixed 2020 date no real transcript can be near, do NOT need the override: the timestamp bound protects location the same way the date already protected blind discovery.

**Backfill: `tokens recompute` locates for completed tasks (task #817, widened #819).** Every task attributed BEFORE this feature landed banked a self-report — or nothing at all, from a pre-D188 binary's terminal marker or a tick that found no evidence — with nothing above it, because the old `compute_attribution` never looked at a transcript for one — item 3 above narrows the live daemon's early return, but a daemon only re-attributes a task it has not attributed yet, so it never revisits them. `token_recompute` (D50 Decision 3's one-shot repair) gains a fifth action, `"locate"`, scoped to exactly the gap the daemon cannot close on its own: a done task holding no measurement other than (at most) a self-report, whose recorded `client` maps to the Claude Code parser — self-reported or not (#819: the original scope only ever used `self_reported` as a cheap way to say "done", which stranded every done task with no measurement at all). A task already carrying an OTLP measurement is excluded even though it has no log-parse row either: `rollup_measurements` only ever supersedes a self-report, never an `otel` row, so a fresh log-parse HIGH row appended beside one would double the roll-up forever, `--apply` being append-only (#819 review, CodeRabbit finding on PR #152). For each, in the same bank-ordered pass `classify_task`'s existing four actions already run in (so an in-scope task's identity claims resolve in completion order, matching what the daemon would have banked), `attribution::locate_backfill` runs the D188 location path — the task's own window (`WindowScan::window_for`), `locate_transcripts_by_task_call`, the SAME located-neighbour contest (`attribution::LocatedNeighbour`) gives the live tick, and the SAME D50 refusal (`totals_in_window_refusing`) against the one running claim set the whole pass shares — and either appends a fresh `source=log-parse`, `confidence=high` row (never rewriting or deleting: D50/D188 stay append-only) or reports `"skipped"` with a `reason` and writes nothing. A second `--apply` is a no-op: `classify_task` gained a narrow parity guard so any HIGH row with no explicit `transcript_path` — a `locate`d row's own shape, self-reported or not — is left exactly alone rather than re-entering a self-report conflict or falling through to the generic "missing evidence" downgrade a row with no path to re-verify would otherwise hit on every later pass. A HIGH row that DOES carry an explicit path is untouched by that guard and keeps today's behaviour.

### D189 — `store.import --merge` takes each scalar field group from the side whose latest event touched it, and counts tracked time from the union of both event logs, instead of taking the whole row from the later `modified` (task #766; amends D185, applies D3 as written)

**Decision:** on a merge of a task this store already holds, the scalars are decided per field group, off the event log. The store side is this store's task events for that id as they stood before the import's event union; the payload side is the document's `events` for that id. A group's winner is the side whose latest event touching any of its columns is later in the **causal order**: by `ts`, except that an event closing a running interval (`stop`, `done`, `cancel`) never sorts before the start it closes. Those events record the instant their interval opened (`interval_started`, written by `task.stop`, the D6 auto-stop, `task.done` and `task.cancel` of a running task), and a close stamped earlier than that instant, which only a machine with a slow clock writes, takes the start's instant and sorts after every other event there. What is left of a tie breaks on event id, the same way on every machine. When the logs cannot decide a group — the very same latest event on both sides while the two copies still hold different values, or a group only one log touches, or neither — the group goes to the copy with the later task-level `modified`, and at equal stamps to the copy whose values sort higher as JSON text: a rule both sides compute identically, so neither direction of a merge favours its own copy. `add` touches every group; `start`, `stop`, `done`, `cancel`, `reopen` and `undo` of a `stop` touch the status group; `modify` touches the groups its `set` keys name; no other op touches a scalar. The per-field rule runs only when BOTH logs hold the task's `add`, the one event that proves a log is the task's history; otherwise the whole row falls back to D185, so a legacy export, a hand-written document, and a store the task reached through one (whose log for it is a lone `import`) all still get exactly D185's behaviour. `created` and `short_id` keep the store's value, `modified` becomes the later of the two (and is written even when nothing else moved, so both copies end on the same stamp), urgency is recomputed from the merged priority and `due`, and the upsert runs only when a merged value differs from the stored one, so a second import of one document writes nothing.

**Field groups** (`engine/field_merge.rs`, `GROUPS`): `status` with `completed`, `active_since` and `delivered_annotation_id`, because `done`, `reopen`, `start` and `stop` write them together and half of each side is a row no call sequence reaches (a `pending` task with a `completed` stamp, a `done` one with an open interval); `due`, `scheduled`, `wait` and `remind` as one `schedule` group, because `task.modify` validates them against each other (#141/#142) and a merge must not pair one side's `wait` with the other's earlier `due`; every other column alone: `title`, `priority`, `project`, `estimate`, `recurrence`, `budget_tokens`, `spawned_from`.

**Tracked time is counted, never chosen, and the count is a symmetric function of the two sides**, so both directions of a merge, and any order of merges among several stores, reach the same figure. It has two parts.

*The history-derived part* is counted over the UNION of both logs, deduplicated by id; `import` events, which `store.export` never carries, are read on neither side, so both directions of a merge see the same two logs. **Every timed interval is its own entity, keyed by the instant it opened, and its length comes from the event that closed it** — the one measure taken on a single machine's clock: a `stop`'s recorded `tracked`, or a `done`/`cancel`'s own instant minus the `interval_started` it names, which is exactly what the closing machine folded into its own total. No length is ever read off the gap between two events stamped by two different machines, so a clock running fast or slow on one of them cannot turn into tracked time. An interval closed on both machines is one entity and counts once, at the longer of its two closes. A `stop` that names no interval (written before closes carried `interval_started`) closes the one the log last opened, with its own recorded length, and one that finds nothing open re-closes the one closed last; a `done` or `cancel` that names none closes nothing, since it was written on a machine with no timer running (or before closes named theirs). An interval that NO close ends is either the timer the merged row keeps running (its `active_since`), which is not counted, or one left running on a machine while another moved the task on: it is banked, unless the next status event after it in the causal order is a `start`. The banked span runs from the start to the FIRST event of any kind after it in the union, which is never later than the next event the start's own machine wrote, and only what lies after the latest reset counts. The bound is read from the union alone, never from which log held what, so several stores merged in any order bank the same span. A new start while an interval is open can only come from a machine that never saw it open, since on the machine that had it open a start writes nothing, so the gap between the two starts spans two clocks and is not counted. `adjust_tracked` adds `delta_seconds` to both columns, `undo` takes a stop or an adjustment back, and an absolute `modify tracked:` is a reset: the count restarts at the value it set, with no adjustment, and the latest reset in the union is the base.

*The residual* of a side is its stored total minus what the same walk counts over that side's own log, never below zero: time the history does not explain, from a legacy row, a task that arrived through a plain import without events, or time from before the log began. The merged total is the history-derived part plus the LARGER of the two residuals, not their sum, because one side may already have received the other's residual through an earlier import and a sum would count it twice. A side that does not hold the union's latest reset has its residual dropped, since that residual predates the reset. `tracked_adjustment_seconds` follows the same rule, and is clamped with the total: the adjustment is part of the total, so when the sum would take the total below zero, the total stops at zero and the adjustment moves by the same amount.

**Reported.** Each `merged` entry gains `from_payload` and `from_store` (the columns that differed, by the side each was taken from) and `tracked_delta_seconds`; `took` keeps its name and becomes `payload`, `store` or `mixed` by which side supplied the differing fields, and D185's side when none differed. `tasqx import --merge` prints `note: merged: task <id> took status, completed from the store and title from the payload; tracked +20m from the payload`, naming only what differed.

**Why:** two machines merged with D185 lost work (#766). A completed task that the other machine later adjusted by 20 minutes came back `pending` with 20 minutes, because B's `modified` was later and every scalar moved with it: A's completion, the only status change on either side, and A's own tracked time were overwritten by a copy that never saw them. D3 already said the policy is per-field last-writer-wins driven off the event log; D185 approximated "per field" with the one row stamp, and the approximation fails exactly when the two machines touched different fields. Tracked time is not a last-writer value at all: each side's total is the sum of its own intervals and corrections, so a merged total that picks one side always drops the other's.

**Limitations.** Banking an interval nobody closed reads the span from its start to the first event after it, and when that event was written on the OTHER machine the span carries the two clocks' skew; it is the one place a cross-machine gap is counted, because the alternative is dropping the time outright. Two machines that each started the task independently lose the stretch between the two starts, which is never banked. Under clock skew the latest event is the latest STAMP: a machine that writes a field after receiving another machine's write to it, stamped later on a faster clock, loses that field to it, and with three stores the order of the first merges can then decide which copy's value stands until the copies meet again and the tie rule settles it. A cancelling `modify` records no `interval_started`, and events written before closes carried one have none either, so a slow clock on such a close still sorts it by its own stamp. Two DISJOINT residuals, legacy time on each side that the other never received, keep only the larger; that is the deliberate price of never counting a residual twice, and it only arises for time recorded before either log began. A `done` or `cancel` that finds no open interval adds nothing, even when another machine stopped the same interval earlier: a single machine completes a pending task without a running timer, and the log cannot tell that apart from a late close, so only a second `stop` extends an interval. A rebuilt fold uses the closing event's `ts`, which `insert_event` stamps a moment after the operation read its clock, so it can differ from the stored fold by a second. `modified` still decides a note both stores hold under one id (D185), since an annotation carries no per-row stamp.

**Rejected: a per-column `modified` stamp.** It would need a schema migration and a stamp write on every mutation, and it would still say nothing about which side's tracked intervals the other has seen; the event log already records both, on every store. **Rejected: the store's total plus the payload's unseen deltas.** It is not symmetric: the side absorbing the other counts differently from the side being absorbed, so the two directions of a merge, and three stores merged in different orders, end at different totals. **Rejected: summing the two residuals.** A residual one store received from the other through an earlier import would be counted twice.

### D190 — `store.import` keeps a local project's own non-empty description over a payload's, and reports every one it dropped (task #767; extends D37)

**Decision:** `upsert_project`'s UPDATE path no longer overwrites `description` unconditionally on a name match. "Blank" means trimmed-empty, on both sides, and nothing else: `opt_str_nonempty` only refuses the literal empty string, so `description: "   "` reaches `upsert_project` as ordinary `Some("   ")`, and a description is only ever "real" when it has a non-blank character after trimming. When the destination already carries a real description for that project, it stands — the payload's is dropped and reported, `{name, dropped}`, under `store.import`'s new `project_description_conflicts` field (always present, empty when nothing was dropped, the same rule `renumbered` and `docs_merged` already follow) — a blank payload description neither overwrites a real local one nor counts as a conflict, since there is no real text on its side to have lost. When the destination has no real description yet, or the payload sent none (or a blank one), there is nothing to drop, so the payload's real text, if it has any, is taken and the import stays silent. `archived` is unaffected: D37's rule that the document is authoritative about it stands, because D22 already treats archiving as a normal, repeatable write that a later import should be able to make either way.

**Why.** A project row carries no `modified` stamp the way a task's does (D185) or a memory doc's does (D183) — `storage.rs`'s `projects` table has only `created` — so there is no later-wins timestamp comparison `upsert_project` could make even if it wanted to. The bug this closes (#767) was `tasqx import`/`--merge` silently replacing a hand-written project description with whatever a second machine's store happened to still carry — including a blank one, on a store that never set it — every time the two synced. With no timestamp to arbitrate, and no rule saying which machine's write is newer, keeping what is already here is the only choice that cannot silently discard a real edit: a payload that never had anything worth keeping (no description, a blank one, or the SAME one) never triggers the rule, and a fresh project (no local row at all) has no local text to protect. Trimming on both sides matters here, not just cosmetically: without it a whitespace-only local description would count as content and permanently block a genuine payload description from ever landing — a description silently WITHHELD, the mirror image of the overwrite #767 reported.

**Rejected: adding a `modified` column to `projects` so a later-write-wins rule could apply, matching D183/D185.** The brief for this fix explicitly ruled it out, and the reasoning holds beyond scope: a migration to grow the row-level protocol every project row already has to obey is a much larger change than a defect closing a two-machine merge that has run this way since D37, and D185's own `merge` flag exists precisely because tasqx does not assume every store carries comparable clocks. Keep-local is the same shape of answer D185 gives a task doc: when there is no reliable "later" to compare, prefer not to discard what a human wrote by hand over what an import happened to also carry.

**Rejected: also gating on `archived`.** `archived` already has a well-understood, repeatable meaning (D22) and un-archiving via import is a documented, intentional feature (`docs/wiki/Projects.md#tasqx-archive`); nothing in #767 or its evidence showed an archived-flag flap, and folding it into the same keep-local rule would silently break that feature for no reported defect.

### D191 — `store.import` folds two copies of one recurrence occurrence into the older, and reports each fold as `deduplicated`; `task.done` spawns nothing into a filled slot (task #776; extends D2, D170, D189)

**Decision:** a recurrence occurrence is `(spawned_from, due, scheduled)`, NULLs equal: one slot of one rule. Two rows on one slot are the same occurrence twice, whoever minted them. `task.done` spawns the next occurrence only when no task already holds its slot, so a reopen and a second completion on one store leave one child. After `store.import` has written its tasks, edges, links and events, inside the same transaction (so `dry_run` rolls it back too), every slot in the store holding more than one task is folded into its lowest id, including a pair the store already held, and the fold repeats until no slot holds two, since re-pointing a dropped copy's successors can fill a slot one generation down. Folding moves onto the survivor the other copy's annotations, checks, token measurements, tags, dependency edges in both directions, link ends, task events and the `spawned_from` of its successors, then deletes the copy. A live note, or a check with the same body, state and evidence, that the survivor already holds is dropped rather than moved. A moved note, check or token row takes a UUIDv8 derived from its original id and the survivor: the original's 48-bit time prefix, the rest XORed with a UUIDv5 of the survivor's id; the XOR is undone first when a row already folded onto one copy is folded again, so every fold path gives the same id (a non-v7 original gets a plain v5 hash instead). The status group (`status`, `completed`, `active_since`, the delivery pin) is D189's: it goes to the copy whose log holds the later latest status-touching event, in D189's causal order and with its tie-break (`field_merge::group_winner`, `tie_side`); every other scalar is the survivor's. `tracked_seconds` and `tracked_adjustment_seconds` are D189's count (`field_merge::merged_tracked`) over the two copies' logs taken as one task's history: what the union counts, each event once by id, plus the larger of the two unexplained residuals. A copy's log is its task events in this store plus every event the document carried for it, since an event this store already held under the other copy was skipped by the event pass. The dropped copy's events then move onto the survivor. An edge from the survivor that would now close a dependency cycle is dropped. The survivor's `rev` goes up by one when the fold changed it; `modified` does not move. Each fold is reported as `deduplicated: [{kept, dropped, spawned_from, kept_id, dropped_id, dropped_status, status, dropped_dependencies}]`, always present, and `tasqx import` prints one `note: occurrence #Y duplicated #X (both spawned from #Z); kept #X` per fold, naming a dropped copy's status when it was `active`, `done` or `cancelled` and any dependency it dropped. The logic lives in `engine/recur_dedupe.rs`.

**Why:** a recurring task completed on two machines spawns the next occurrence on each under different ids, and importing one store into the other left two copies of one piece of work (#776). `spawned_from` alone is not the key: the date says which turn of the rule a child is. Folding every duplicate slot, not only one the import brought a member to, is what makes the stores converge: a pair one store holds and the other does not would otherwise stay on one side for good while the other refolded on every import, and a same-date pair is a duplicate by this definition whichever store made it; spawning once per slot keeps a store from making one. The lowest id, D189's status rule and D189's time count are all symmetric in the two copies, so the result is the same in both stores whichever direction runs first, a stop or a completion on either copy stays the last word wherever it lands, and re-importing a file folds the returning copy again with no tombstone. Counting time from the events rather than adding the two totals means an interval both logs hold counts once, and work logged on a copy after another store folded it still arrives through its new events. Moved rows change id because a note, check or token id held by another task refuses the import (#802, #803), and the other store still holds the old ids under its own copy. The `rev` bump keeps the #177 guard from letting a pre-fold export overwrite rows the fold moved in. A cycle edge is dropped rather than the import refused because a refusal would repeat on every later sync.

### D192 — the README opens on a real, unedited agent session that checks before it says done; the SVG map moves below the fold (task #822; amends D176, follows D175)

**Decision:** the first picture under the README's wordmark is `docs/img/hero-receipt.gif`, one take of a real Claude Code session: a clean config directory with tasqx as its only MCP server, the invented demo store copied to a scratch file, and one typed prompt, "The SDK 3.0 migration guide is written. Close its task." In the committed take the agent reads #51 through tasqx, finds it blocked by #50 with two acceptance checks open (the guide it was pointed at still says "The ceiling is TBD"), and asks "Close #51?" rather than reporting done, offering a forced close that `tasqx_outcomes` would count as forced. Three scripts make it: `scripts/hero-stage.sh` builds the room (store, an invented repository holding the guide, the config directory, the MCP config), `scripts/hero-receipt.tape` records the take with VHS, and `scripts/hero-assemble.sh` cuts it and adds an end card with the install line. The rule for a take: retakes are allowed, an edit to the agent's text is not, and time is the only thing the cut changes — the stretch where the agent works plays faster under a label on the frame that says so. D176's `docs/img/hero.svg` moves to the top of "What the organiser holds", the section it maps. D163's `docs/img/hero.gif` and `scripts/hero.tape` stay as they are: still listed in `docs/maintainers/terminal-style.md` §14, still linked from nowhere.

**Why a real session and not a tape.** The pitch since D175 is what tasqx does for an agent, and the claim a visitor has to believe is about the agent's behaviour. A VHS tape can type CLI commands honestly because the binary answers them; it cannot type an agent's answer without writing it, and a written answer is staged. So the model answers for itself, and what the scripts fix is only the room it walks into. That is also why a take is not byte-reproducible while the room is: the stage script rebuilds the same store, repository, commit hash and config on every run, but the model's answer varies between runs, and holding it still would mean editing it.

**Why "the agent checks before it says done", and not "tasqx makes it prove it".** tasqx does not make an agent prove anything. D138 counts an unproven completion rather than refusing it, and a forced close goes through; D150 refuses `done` on an open blocker, but in the committed take the agent never called `tasqx_complete_task`, because what `task.get`/`task.brief` returned already showed the blocker and the open checks. The honest claim is the one the take shows: the receipt was there to read, the agent read it, and it asked. A caption promising enforcement would be contradicted by the first forced close a reader tries.

**Rejected: an amnesia A/B against a bare session** (the same task in a session without tasqx, forgetting what the previous one decided). It loses to "just use CLAUDE.md", which is the answer any reader already has for memory, and a comparison between two model runs invites scenario-shopping: rerun both until the contrast looks good, which is the staging this ruling forbids.

### D193 — a plain `memory.search` that finds nothing holding every word falls back to any word, and says so with `relaxed: true` (task #826; extends D69 and D136(a), reverses neither)

**Decision:** the all-words search stays the first one run. A plain (non-`raw`) query whose AND finds nothing in the requested scope runs once more: its words, quoted by the same `phrase` rule the AND uses (the escaping D41 needs), minus the brief's `QUERY_STOPWORDS` (now shared from `engine/memory.rs`), minus a word already taken in another case, and minus a word with no letter or digit, joined with ` OR `, bm25-ranked, with the same `scope`, `project`, `include_unscoped` and `limit`. The fallback does not run when nothing is left, or when one word is left and the AND was that word alone, because then the OR is the search that already ran (`release Release`, `of the with`, `zebra`). It is its own match mode in the engine (`MatchMode::AnyWord`), not a `raw` search: tasqx wrote the expression, so a failure running it is `internal`, and the raw-mode error hints never apply to it. The result gains `relaxed`, always present like `matched`: `true` when the fallback ran, `false` otherwise. `matched` is the expression that produced the returned hits, so under `relaxed: true` it is the OR, and `total`/`has_more` count the OR's matches. When the OR misses too, the answer is zero hits with `matched` the OR and `relaxed: true`: the widest expression that ran is the one that tells the caller dropping words cannot help, where the AND echo would advise exactly that. `raw: true` never falls back, and a single word has nothing weaker to fall back to. `tasqx memory search` prints `no hit had every word — these match any word` under relaxed hits, and on a relaxed miss names what it searched (`nothing in docs or annotations has any of these words: <expr>`, or `no doc has` / `no annotation has` under `--scope`). A plain miss that did not relax had one content word or nothing the OR could add, so its note no longer says "use fewer, or --raw with OR" but `nothing matched <expr> — try other words, or --raw with word* for a prefix`. The MCP tool description, the server instructions, the in-binary method doc, the wiki's Memory page and the agent starter prompt say the same.

**Why.** Agents write two- and three-keyword queries, as the instructions ask, and those keywords routinely span entries: an agent asked `SDK 3.0 release` against a store holding a doc that says "SDK release" and an annotation that says "3.0 release", got `count: 0`, and did not retry, although the server instructions tell it to try a second wording. An instruction a real session ignored is not a mechanism. AND stays right as the first reading of a caller's words (D136(a) calls a caller's query a statement of what they want, which is why the brief builds its own disjunction rather than rewriting the caller's), so the any-word search is only what answers when the all-words one has nothing. It is visible, per D69: `matched` names the OR and `relaxed` says why it is an OR, so every result still says what it answered about. That is the line D136 draws against a silent scope widening (a second search the caller cannot see) and D147 against a rank boost (a mix with no account of it); a partial match is never passed off as a full one. The stopwords go because a filler word is in nearly every entry: an OR that kept `the` answered `the zeppelin hangar` on a store with nothing about zeppelins with every entry, and a miss that should have read as "nothing stored on this" (D69) read as a page of noise.

**Known limit: relaxed hits are ranked by plain bm25 over the OR.** A hit holding one rare word can outrank one holding three common ones; bm25 already sums per-term scores, so entries matching more words usually lead, and a boost by matched-term count is the kind of unexplained rank change D147 rejected.

**What does not change.** `task.brief`'s derived search is already a disjunction run `raw`, goes through `memory_search_excluding` and never reports `relaxed`; its result shape is untouched. `graph.query`'s `search_match` edges stay all-words, also through `memory_search_excluding`: an edge claims the root's title words are in that entry, and one shared word does not back the claim. The fallback widens the words and never the scope, and `raw` callers keep the grammar they wrote.

### D194 — the README's reference section opens on a second real agent session, one that looks up a recorded decision before it acts (task #825; follows D192, D193)

**Decision:** "The reference section" in the README's "What the organiser holds" opens on `docs/img/memory-recall.gif`, one take of a real Claude Code session in D192's room and under D192's rules: the same clean config directory, tasqx as the only MCP server, the invented demo store, one typed prompt, retakes allowed and edits not, time the only thing the cut changes. The prompt is "Cut the SDK 3.0 release." In the committed take the agent calls `tasqx_search_memory` with `SDK 3.0 release`, reads the stored `release-process` doc (cut the branch on Monday, tag after a day of canary) and the ruling annotated on #51 (the migration guide ships *with* the 3.0 release), says the release is blocked and that today is not the day the schedule names, and asks how to proceed rather than cutting anything. The tape-recorded `docs/img/memory.gif` stays beside the section's text: it shows the human side, `memory import` and `memory search` at the shell, which the session does not. `scripts/memory-recall.tape` is `scripts/hero-receipt.tape` with the prompt changed; `scripts/hero-assemble.sh` takes `HERO=receipt|memory`, which names the GIF it writes, when its tape finishes typing the prompt, and the two headline lines of its end card, and defaults to `receipt`, so D192's pipeline runs unchanged.

**Why this scenario.** The claim the section makes is that what an earlier session decided is there for the next one, so the take needs a decision the agent could not have guessed and would act against without it. Two stored things are needed and neither suffices alone: the ruling on #51 is an annotation, which the task list does not show and a CLAUDE.md does not hold, and the release process is an imported doc. The prompt is an ordinary request to do work, not a question about memory: an agent asked "what did we decide about the release?" searches because it was told to, and that proves nothing about whether it would look before acting.

**The first dry run missed, and the fix went into the product.** Its agent searched `SDK 3.0 release`, got `count: 0` because no single entry holds all three words, and did not try a second wording. It still found the ruling, because it happened to open #51 with `task.get`, but it never saw the release process, so half of what the session was meant to show came from luck rather than from memory. Rewording the prompt until the search hit, or seeding the store with an entry that happens to hold all three words, would have made the take show a search that works only in that room, which is the staging D192 forbids. The miss was a product defect, and it was fixed as one: D193's any-word fallback, merged before this take was recorded, is why the same query now returns hits under `relaxed: true`, and why the recorded agent opened the release-process doc by an id only those hits carry. The prompt is the one the dry run used.

**Re-record** after a change to #50 or #51 and their notes in `scripts/demo-store.py`, to its `MEMORY` docs, to what `memory.search` returns, or to the MCP tool surface (`docs/maintainers/terminal-style.md` §14).

### D195 — the README opens on both real sessions, the receipt first and the memory recall directly under it (task #825; amends D192, D194)

**Decision:** under the wordmark the README shows `docs/img/hero-receipt.gif` and then `docs/img/memory-recall.gif`, both above the tagline, each with its provenance comment. D192's rules and D194's take are unchanged; only the placement moves. "The reference section" returns to its layout before D194: its text beside the tape-recorded `docs/img/memory.gif`, the shell side of memory.

**Why.** The maintainer's call: the two sessions together make the pitch. One shows the agent checking before it says done, the other shows it looking up what you decided before it acts, and a visitor should see both before scrolling. Deep in "What the organiser holds", the second one reached only readers who had already scrolled past the claim it backs.

### D196 — Memory search is hybrid: a static embedding model built into the binary ranks by meaning beside FTS5's bm25, fused by reciprocal rank, every hit saying how it was found (tasks #835–#839; amends D41 and D69, extends D136, D154, D193)

**Decision.** `memory.search` gains a semantic retriever and, by default, fuses it with the lexical one D41 built.

- **Model.** MinishLab's `potion-base-8M` (Model2Vec, MIT; vocabulary from bge-base-en-v1.5, MIT): a 29,528 × 256 static embedding table. It is stored in the tree as a per-row int8 matrix with one f32 scale per row plus its WordPiece vocabulary (about 7.8 MB together), built from a pinned upstream revision by `scripts/embedding-model.py`, which checks the download's SHA-256, and compiled into `tasqx-core` with `include_bytes!`, parsed once per process on first use. `NOTICE` carries the attribution and licence texts, ships in every release archive, and is compiled into the binary for `tasqx about --notices`, so an install that kept only the binary still carries it. The model id is a constant naming the table, its revision, the quantisation AND the tokenizer and chunker versions (`potion-base-8M@<rev>/int8/tok1/chunk1`): a change to any of them is a new id, so vectors built by an older chunker are never mixed with newer ones.
- **Inference is ours, in pure Rust.** A BERT-style WordPiece tokenizer (clean control characters, space around CJK, lowercase, strip accents via NFD, split on whitespace and punctuation, greedy longest-match `##` subwords, a word over 100 characters is `[UNK]`), then the mean of the token rows, L2-normalised, summed in one fixed scalar order. `[UNK]` and special tokens are dropped; text with no known token has no vector. Nothing is truncated: long text is chunked instead. A golden test pins token ids and vectors against the upstream `tokenizers` and `model2vec` output for a committed sample. Its Unicode handling — what is dropped, spaced, lowercased or stripped of an accent, what counts as whitespace or punctuation — is not Rust's or any crate's but a table `scripts/embedding-model.py` measures from the upstream tokenizer one code point at a time, checked by a test against upstream checksums over every code point, because upstream's Unicode version is its own and a local table drifts from it. So no crate is added, and nothing native.
- **What is embedded.** For a doc, its `title` and its `search_body` (the body with front matter flattened, D135); for an annotation, its `body`, when `removed` is null and the body is not empty. An annotation is one chunk unless it is over 150 words. A doc is split on blank lines and headings into chunks of about 40–150 words (a CJK ideograph counting as one, an unbroken run as one per 25 characters, so no text escapes the bound): short neighbours merge, a long paragraph splits on sentence ends with one sentence of overlap, a fenced code block is never split. Every chunk of a doc is prefixed with the doc's title and heading path. An entry scores as its best chunk.
- **Storage.** `memory_vectors(kind, owner_id, model, chunk, vec)`, primary key `(kind, owner_id, model, chunk)`, `vec` the 256 int8 values plus their f32 scale (260 bytes), or NULL on a single sentinel row (`chunk` −1) for an entry that has no known token, so it is not re-embedded on every search. No copy of the text is stored: a semantic hit's snippet is cut from its source row by re-chunking it. Other models' rows are ignored by search and deleted by `migrate` on open, never by a search, so two binaries with different models on one store do not delete each other's work on every call.
- **Freshness.** Pure-SQL triggers delete an owner's vectors, for every model, when a doc's `title` or `search_body` changes, when an annotation's `body`, `id` or `removed` changes (each `WHEN old.x IS NOT new.x`, so an upsert that changes nothing keeps its vectors), and when either row is deleted; inserts do nothing. Before a search that uses meaning, the engine finds every entry with no row under the current model, embeds it, and writes it with `INSERT OR IGNORE … SELECT … WHERE EXISTS` on the source row still holding exactly the text that was embedded (the doc's `title` and `search_body`, the annotation's `id`, `body` and `removed IS NULL`). A write that lost a race with an edit therefore inserts nothing, and the next search embeds the new text; a vector can never describe text its row no longer holds. The write is attempted without waiting on the lock: if another writer holds it, or the file is read-only, the missing vectors are computed in memory for this call only, and a search never fails because of the vector index. So every write path — `memory.add/import/update/refresh/remove`, `annotation.add/update/remove`, undo, `store.import` in both modes, D191's fold, a recurrence spawn — and every writer — the daemon, an in-process CLI, each MCP server, an older tasqx binary on the same file — leaves the index correct by doing only the SQL write it already does. This makes `memory.search` write derived data; it changes no entry and no task, so it stays a read under the MCP server's read scope. Vectors are derived: `store.export` does not carry them and `store.import` never needs them.
- **Cost.** Each process keeps the current model's vectors in memory, reloaded when `PRAGMA data_version` says another connection committed. A warm search at 50,000 entries stays under 50 ms and a cold one under 1 s in a release build; #839 measures both.
- **Search.** `memory.search` takes `mode`: `hybrid` (the default), `lexical` or `semantic`, and `min_similarity`, a number from 0 to 1, default 0.30. `raw: true` is lexical by definition and is refused beside `mode: semantic` or `hybrid`. The semantic side runs only when the query has a content word — at least three letters and not in `QUERY_STOPWORDS` — so an identifier such as `D41` or `#607` is looked up by its characters alone. Each list is taken to depth max(50, `limit`): the lexical list exactly as D41, D69 and D193 make it, except that in hybrid mode the relaxed any-word OR runs only when the semantic side found nothing at or above the floor (otherwise meaning already answered and the OR would only add noise, and `relaxed` stays `false`); the semantic list is every entry in the requested `scope`, `project` and `include_unscoped` whose best chunk's similarity, rounded to three decimals, is at or above `min_similarity`. The two are fused by reciprocal rank, score = Σ 1/(60 + rank); ties go to `both` before `lexical` before `semantic`, then to the id. `total` is the size of the union of every lexical match and every semantic match at or above the floor, so `has_more` still means what #132 made it mean.
- **Every hit says why it is there.** Each hit gains `via` (`lexical`, `semantic` or `both`), `similarity` (its best chunk's cosine to three decimals, or `null` when the entry has no vector or the semantic side did not run) and `score` (the fused score, higher is better). `rank` keeps its contract — a number, lower is better, not a threshold (D154) — and is now the negated fused score, so a v1 client sorting ascending by `rank` still gets the order the result is in; it is no longer the raw bm25 value, which a semantic-only hit does not have. A semantic-only hit's `snippet` is its best chunk. The result keeps `matched` and `relaxed` for the lexical side and gains `semantic`: `{model, min_similarity}` when the semantic side ran, `null` when it did not. D69's promise that `matched` explains every hit now reads: `matched` explains every hit with `via` `lexical` or `both`, and `similarity` every hit with `via` `semantic`.
- **Where else.** The brief's and `start`'s derived memory (D136, D186) keep their raw lexical expression and add a meaning query from the task's title, tags and project, through the engine's own path (the refusal of `raw` beside hybrid is for callers); the task's own notes are excluded from the semantic list as from the lexical one (#607), before the limit and before D147's half-for-docs split. `graph.query`'s title-word edges stay lexical: an edge claims the entry has the title's words. `tasqx memory search` takes `--mode` and `--min-similarity` and marks a semantic-only hit; the MCP tool takes `mode` and `min_similarity`; the tool text, the server instructions, the in-binary method docs, the wiki and the starter prompt say that search matches meaning as well as words, within D155's roster budget re-measured.

**Why now, and why this model.** D41 ruled out embeddings for three reasons: a runtime (`ort`) that multiplies the binary and the dependency tree, a model download at first use that breaks offline and no-signup use, and no standard way for clients to supply vectors. A static embedding model answers all three: no network, because the table is in the binary; no runtime, because inference is a lookup and a mean; no added crate. What it costs is size, about +8.3 MB on a ~10.6 MB binary, which this ruling accepts: memory that finds "login" when the note says "sign-in", or the release process when an agent asks when to ship, is what the organiser is for, and #826 showed agents' queries missing on words when the answer was stored. potion-base-8M beat potion-retrieval-32M on a measured sample at a quarter of the size; transformer models (MiniLM, bge-small) retrieve better but bring back the runtime and cost milliseconds per text instead of microseconds.

**Why hybrid and not semantic alone.** Exact words — ids, file names, error strings, `D41` — are what bm25 is good at and static embeddings are bad at; meaning is the reverse. Reciprocal rank fusion needs no calibration between the two scores, and every hit carries its reasons, which is the difference from the rank boost D147 rejected.

**Why triggers that only invalidate, and a guarded write.** A trigger cannot embed (SQL has no model), a function registered on the connection would make every older binary's writes fail with "no such function", and a cache in one process misses the others' writes. Deleting on change, embedding what is missing at search time, and inserting only while the source row still holds the embedded text is the one design under which a writer that has never heard of vectors still leaves them right, including an edit that lands between another process's read and its write.

**Known limits.** The model is English: Dutch text matches mostly where query and note share words. Static embeddings separate related from unrelated text weakly, so the floor trades some paraphrase recall for silence on unrelated queries; it is a parameter, and #839 measures recall at 0.30 on real stores. `similarity` is rounded to three decimals before the floor and before ranking so that last-digit differences between CPU architectures cannot move a hit. The table is about 7.8 MB of git history per model bump, every test binary links it, and if `tasqx-core` is ever published to crates.io the table goes into the crate (under its 10 MB limit, to be measured then).

**Rejected:** `fastembed`/`ort` and `candle` transformer models (runtime, size, per-text cost; D41's reasons stand for them); `model2vec-rs` (55 new crates including a C++ build step through `esaxx-rs`, and it truncates at 512 tokens silently); `sqlite-vec` (a C extension, pre-1.0, brute force anyway at these sizes); the multilingual potion model (128 MB even as int8); cutting dimensions to shrink the table (measured top-1 loss); a config-file floor (the CLI resolves config, so the daemon and each MCP server would disagree about it); keeping bm25 in `rank` (a semantic-only hit has none, and a mixed column would sort wrong).

**Refinements from building #837.** The per-process copy of the vectors is keyed on a memory generation, not on `PRAGMA data_version`: `memory_generation` is one counter that pure-SQL triggers bump when a doc or note is inserted or deleted, when a doc's `title`, `search_body`, `project` or `id` changes, when a note's `body`, `id`, `task_id` or `removed` changes, and when a task's `project` or `id` changes or the task is deleted. `data_version` moves on every commit, so a timer, a tag or a token count threw the copy away; the counter moves only on what a semantic list can see, still from every writer, an older binary included, because the triggers live in the file. Another model's rows are no longer deleted on every open, which let a CLI and an MCP server built with different models delete each other's vectors turn by turn: `memory_vector_models` records when each model last searched the store (written at most once a day), and open deletes only the rows of a model that has not searched for seven days, never the current model's; it reads that small table first and touches `memory_vectors` only when a model is actually stale, since the table has no index on `model`. Vectors a pre-release build of #837 wrote without recording their model are recorded as used at the upgrade, so the same rule sweeps them later rather than at once. The table, triggers and sweep run in one `BEGIN IMMEDIATE`, so concurrent opens cannot interleave. When the guarded write cannot be made, the vectors stay in the copy and the write is retried on a later search without embedding them again; on a read-only store the copy keeps the vectors alone and nothing waits to be written. When memory changes during a reconcile, what was embedded is still written (the guard refuses exactly what changed) and the reconcile starts over, so a busy store still warms up; if it has not settled after a few passes, or on an error, the search gets no semantic candidates, never stale ones. A search inside a caller's open transaction keeps nothing under a key, because a rolled-back generation can be reached again by another commit. Similarity is computed with the query in i16 fixed point and an exact integer dot product against the stored int8 values, one f64 division at the end: order-independent, so it vectorises and gives the same bits on every CPU, and within 1e-4 of the f32 formula on the golden set (a test holds it there). `vectors::tests::cost_at_fifty_thousand_entries` (ignored; run in release) measures warm and cold search at 50,000 single-chunk entries and at 50,000 three-section docs against the budget above.
