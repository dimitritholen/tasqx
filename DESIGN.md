...
| `tasqx_add_tokens` | W | `ref`, `tool`, `source`, `confidence`, the four counts or `total_tokens`, `model?`, `idempotency_key?` | `{short_id, measurement}` — spend recorded after the fact; `total_tokens` is one unsplit count, never beside the four (D167) | `token.add` |
