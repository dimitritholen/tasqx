...
`report.outcomes` (**D137**) has shipped and is in the table above's namespace list. Ruled and not yet built, each with its own entry in §12: `task.brief` (**D136**), `check.add` / `check.set` / `check.remove` (**D138**). All three rulings are additive under the frozen `"tasqx":"1"` API — new methods and new params on `task.done`, no existing response field renamed or removed (D56). D139 adds no method at all: `budget_tokens` is a field on `task.add` / `task.modify` and a pair of derived keys on `task.get`.
...
**Scheduled, ruled, and not in the roster above.** Three §12 entries add to this surface and none of them has been built; the table stays the shipped set, because the guide and the README derive their rosters from the running server and a row here that the server does not serve is a roster with two truths. (D137's `tasqx_outcomes` was the fourth and has shipped, so it is in the table itself.) `tasqx_brief_task` (R, `task.brief`, **D136**) answers "what do I need before starting this" in one call — the task, its dependency neighbourhood with the prerequisite's newest annotation, and memory hits under a query tasqx derives rather than the caller guesses. `tasqx_add_check` / `tasqx_set_check` / `tasqx_remove_check` (W, `check.*`, **D138**) carry acceptance criteria as stateful rows with an `evidence` citation tasqx stores and never interprets, and `tasqx_complete_task` grows `checks_passed[]` and `evidence?`. Under **D139** `tasqx_add_task` and `tasqx_modify_task` grow `budget_tokens`, and the task reads report `fresh_tokens` and `over` beside it. `tasqx_brief_task` is on the **read** scope on purpose, for the reason `tasqx_search_memory` and the shipped `tasqx_outcomes` are: an agent that cannot write should still be able to orient itself and to see its own record.
...
>
> **Not yet built:** the bare-ref command form (`tasqx 42 +blocking`, `tasqx 42 done`) and the fuzzy verb matching beside it; the rest of §5's REF grammar (ranges, comma lists, `@active`/`@last`); D8's `.is` suffix and saved-filter expansion (`@overdue`, `@today`); the socket-client daemon auto-spawn half of D5; the no-daemon OS-scheduler path and actionable toast buttons (§9b); plugins and hooks (§6); sync (D3). And three of the four agent-efficiency rulings, decided in §12 and scheduled: `task.brief` (**D136**), acceptance checks (**D138**) and token budgets (**D139**). The fourth, `report.outcomes` (**D137**), has **shipped**, and it was built first on purpose: it reads history already in the store, so it produced a baseline retroactively rather than starting a measurement period, and every claim the other three make — fewer setup tokens, less rework, better-sized tasks — is a claim it is the instrument for. Building any of them first would have shipped an improvement that could only be asserted. **D140** is ruled ahead of all four and for a different reason: it is not an improvement but a correctness fix — under parallel agents D6's auto-stop silently truncates the `tracked` column D137's calibration metric is computed from, and an instrument built over a column that is quietly wrong is worse than no instrument.
