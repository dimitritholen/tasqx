...
**Test-isolation note.** `default_roots` always appends the two standard locations under the real `$HOME`/`%USERPROFILE%` regardless of a `$CLAUDE_CONFIG_DIR` override, by design, so overriding the config dir alone does not stop a scan from also reading the developer's own `~/.claude`. That was a low-probability flake while discovery matched only a date window; location-by-call matches on task-ref CONTENT with no date filter at the file-selection stage, so `attribution::tests::with_isolated_roots` now moves `HOME`/`USERPROFILE` too, serialised under the existing `DISCOVERY_ENV` mutex — measured against this machine's own Claude Code history, which holds a real `tasqx_complete_task` call for ref "1", the short_id nearly every fixture in that module uses.
