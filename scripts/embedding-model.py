...
def write_tables(tokenizer):
    ...
    space, cjk, drop, punct = set(), set(), set(), set()
    for c in range(0x110000):
        ...
        if out == "":
            ...
            space.add(c)
        elif len(out) >= 3 and out[0] == " " and out[-1] == " ":
            cjk.add(c)
        if len(pre.pre_tokenize_str("a" + ch + "a")) == 3:
            punct.add(c)
    doc = {
        "SPACE": "Characters the normalizer turns into a plain space before anything else.",
        "CJK": "Characters the normalizer surrounds with spaces, so each is a word of its own.",
        "DROP": "Characters the normalizer removes: controls, format characters, private use, and\n/// the non-spacing marks left over once NFD has taken an accent off its letter.",
    ...
    for name, points in [("SPACE", space), ("CJK", cjk), ("DROP", drop), ("PUNCT", punct)]:
        rs = ranges(points)
        lines.append(f"/// {doc[name]}")
        lines.append(f"pub(super) const {name}: &[(u32, u32)] = &[")
        lines.extend(f"    (0x{a:X}, 0x{b:X})," for a, b in rs)
        lines.append("];")
        lines.append("")
