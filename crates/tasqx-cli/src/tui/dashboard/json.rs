    ...
    json!({
        ...
        "tokens": {
            ...
            "rows": d.tokens.rows.iter().map(|r| json!({
                ...
                // absent when `report.summary` sent none. Every bucket above
                // is already a blend across measurements, so this is the one
                // field that answers "how sure are we" at all.
...
mod tests {
    ...
    fn tokens_rows_carry_their_confidence() {
        ...
        let d = build(
            ...
            Timestamp::from_second(1_754_000_000).unwrap(),
            date(2026, 8, 5),
        );
        let doc = document(&d, 7, &[]);
        assert_eq!(
