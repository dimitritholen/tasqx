...
///
/// BURNDOWN, not NOW or PROJECTS: PROJECTS answers its own `⏎` now (#204, see
/// `enter_on_projects_asks_the_loop_to_list_that_project`), and NOW answers
/// its own `⏎` too as of #228.13 (see `enter_on_now_opens_the_running_tasks_detail`)
/// whenever a task is actually running. BURNDOWN has no selectable row at all
/// (`model::row_count`), which is what this test means to cover.
#[test]
