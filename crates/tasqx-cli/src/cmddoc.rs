...
pub const COMMAND_REF: &[CmdDoc] = &[
    ...
    CmdDoc {
        ...
        notes: &["Canonical JSON; a filtered export trims edges leaving the set and reports `dropped_dependencies` (D12).",
                 ...
                 "A filter narrows `projects`/`docs`/`events` to what the exported tasks need — an unfiltered export still carries all of them; `--include-unscoped` widens a filtered one back to docs with no project (D171)."],
        see_also: &["import", "api"],
        topic: Topic::JsonApi,
    },
    CmdDoc {
