...
pub const COMMAND_REF: &[CmdDoc] = &[
    ...
    CmdDoc {
        ...
        notes: &[
            ...
            "The pass runs two phases: rewrite D50-refusal rows (`source=log-parse` only, self-report/OTLP never touched), then locate the transcript of every done Claude Code task with no measured (log-parse or OTLP) row and append a HIGH measurement if found; tasks without a transcript are reported as `skipped`.",
