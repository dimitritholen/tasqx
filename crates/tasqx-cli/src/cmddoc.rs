...
pub const COMMAND_REF: &[CmdDoc] = &[
    ...
    CmdDoc {
        ...
        usage: "tasqx memory <add <title> <body> [--source s] [--project p] [--standing]|search <words…> [--limit n] [--scope s] [--raw]|list [--limit n] [--offset n] [--project p] [--standing]|show <id>|update <id> [--title t] [--body b] [--source s] [--project p] [--standing true|false] [--expected-rev n]|rm <id>|import <path> [--project p]>",
        examples: &[
    ...
    CmdDoc {
        ...
        notes: &["Canonical JSON; a filtered export trims edges leaving the set and reports `dropped_dependencies` (D12).",
                 ...
                 "The document carries the graph's explicit links too, so a restore gives back the edges and not only the nodes; a filtered export keeps a link only when BOTH its ends are in it, and counts the rest as `dropped_links` (D180)."],
        see_also: &["import", "api"],
        topic: Topic::JsonApi,
    },
    CmdDoc {
        ...
        notes: &["A task whose number a DIFFERENT task here already holds keeps its id and takes the next free one; every move is printed and reported as `renumbered` (D177). A task this store already holds keeps the number it has here.",
                 "Links are restored after everything they point at, counted in `links_imported`; a link naming an end neither the document nor this store holds refuses the whole import by name (D180)."],
        see_also: &["export", "api"],
        topic: Topic::JsonApi,
    },
    CmdDoc {
