...
pub fn score(priority: Option<Priority>, due: Option<&str>, created: &str) -> f64 {
    let u: f64 = breakdown(priority, due, created)
        .iter()
        .map(|(_, v)| v)
        .sum();
    (u * 10.0).round() / 10.0
...
mod tests {
    ...
    fn priority_weights_are_pinned() {
        ...
        for (prio, want) in [
            ...
            close(
                term(&breakdown(prio, None, &created), "priority"),
                want,
                1e-9,
            );
        }
    ...
    fn due_proximity_ramps_linearly_across_the_fourteen_day_horizon() {
        ...
        let due_term = |days: f64| {
            term(
                &breakdown(None, Some(&days_from_now(days)), &created),
                "due_proximity",
            )
        };
