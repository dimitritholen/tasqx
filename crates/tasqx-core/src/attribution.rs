...
pub struct LocatedNeighbour {
    ...
    pub short_id: i64,
    /// The neighbour's store id, the other spelling.
    pub task_id: String,
    /// The neighbour's `(window_start, window_end)`.
    pub window: (String, String),
    /// The neighbour's explicit transcript path, canonicalized, when it had one.
    pub canon_path: Option<PathBuf>,
}

/// The foreign windows for a task whose samples come from the `located`
        ...
        located_neighbours
            ...
            .filter(|n| {
                n.canon_path.as_ref().is_some_and(|c| canon.contains(c))
                    || located.iter().any(|f| {
                        tokens::claude_code::contains_task_call(
                            f,
                            &n.short_id.to_string(),
                            &n.task_id,
                            &n.window.0,
                            &n.window.1,
                        )
...
pub fn compute_attribution(
    ...
    let (samples, transcript_parsed, session_correlated, foreign_windows) = match pa
        .transcript_path
        .as_deref()
    {
        Some(path) if !path.is_empty() => {
            ...
            (samples, true, correlated, pa.foreign_windows.clone())
        }
        ...
        _ => {
            ...
            if located.is_empty() {
                (
                    discover_samples(parser, pa),
                    false,
                    false,
                    pa.foreign_windows.clone(),
                )
            } else {
                let mut samples = Vec::new();
                for f in &located {
            ...
            for info in correlated.values_mut() {
                if let Some(tp) = &info.transcript_path {
                    let canon = canon_memo
                        .entry(tp.clone())
                        .or_insert_with(|| canonical_path(Path::new(tp)));
...
impl WindowScan {
    ...
    fn located_neighbours_for(&self, task_id: &str) -> Vec<LocatedNeighbour> {
        let Some(info) = self.correlated.get(task_id) else {
            return Vec::new();
        };
        let Some((ws, we)) = self
            .window_for(task_id)
            .and_then(|(ws, we)| parse_window(ws, we))
        else {
            return Vec::new();
        };
        self.windows
            .iter()
            .filter(|(other_id, os, oe)| {
                other_id != task_id
                    && !shares_sample_source(info, &self.correlated[other_id])
                    && parse_window(os, oe).is_some_and(|(os, oe)| os <= we && oe >= ws)
            })
            .filter_map(|(other_id, os, oe)| {
                let (short_id, _) = self.meta.get(other_id)?;
                Some(LocatedNeighbour {
                    short_id: *short_id,
                    task_id: other_id.clone(),
                    window: (os.clone(), oe.clone()),
                    canon_path: self.correlated[other_id].canon_path.clone(),
                })
            })
            .collect()
    }
...
pub fn pending_attributions(engine: &Engine) -> Result<Vec<PendingAttribution>, ApiError> {
    ...
    for (task_id, info) in scan.correlated.iter() {
        ...
        let consumed_sample_ids: HashSet<String> = consumed_by_task
            ...
            .collect();
        // Only a task D188 location can reach needs its unsourced overlapping
        // neighbours; everyone else skips the per-task window pass.
        let locatable = info.transcript_path.is_none()
            && info.client.as_deref().and_then(parser_for) == Some(Parser::ClaudeCode);
        let located_neighbours = if locatable {
            scan.located_neighbours_for(task_id)
        } else {
            Vec::new()
        };
...
mod tests {
    ...
    /// D50 for located transcripts (PR #149 review): two concurrently active
    /// tasks in ONE Claude Code session carry no `transcript_path` or
    /// `session_id`, so the done-payload join never saw them share a source.
    /// Once each is located to the same file, that file IS their shared
    /// source: samples inside both windows bank for neither, the rest bank
    /// normally.
    #[test]
    fn two_tasks_located_to_one_session_contest_their_overlap() {
        let engine = Engine::open_in_memory().unwrap();
        let mut ids = Vec::new();
        for title in ["a", "b"] {
            let sid = engine.task_add(&json!({ "title": title })).unwrap()["short_id"]
                .as_i64()
                .unwrap();
            engine.task_start(&json!({ "ref": sid })).unwrap();
            engine
                .task_done(&json!({ "ref": sid, "client": "claude-code" }))
                .unwrap();
        ...
        // A: 10:00-10:30, B: 10:20-10:50 — they overlap on 10:20-10:30.
        for ((_, id), (started, completed)) in ids.iter().zip([
            ("2026-07-24T10:00:00Z", "2026-07-24T10:30:00Z"),
            ("2026-07-24T10:20:00Z", "2026-07-24T10:50:00Z"),
        ]) {
            engine
                .conn()
                .execute(
                    "UPDATE events SET payload = ?1 WHERE entity_id = ?2 AND op = 'start'",
                    (json!({ "interval_started": started }).to_string(), id),
                )
                .unwrap();
            engine
                .conn()
                .execute(
                    "UPDATE events SET payload = ?1 WHERE entity_id = ?2 AND op = 'done'",
                    (
                        json!({ "completed": completed, "client": "claude-code" }).to_string(),
                        id,
                    ),
                )
                .unwrap();
        ...
        let dir = std::env::temp_dir().join(format!(
            "tasqx-attr-located-contest-{}",
            crate::clock::uuid_v7()
        ));
        let proj = dir.join("projects").join("proj");
        std::fs::create_dir_all(&proj).unwrap();
        let usage = |ts: &str, id: &str, input: u64| {
            json!({"type": "assistant", "timestamp": ts, "message": {"id": id, "usage": {
                "input_tokens": input, "output_tokens": 0,
                "cache_read_input_tokens": 0, "cache_creation_input_tokens": 0}}})
            .to_string()
        };
        let complete = |ts: &str, id: &str, r: i64| {
            json!({"type": "assistant", "timestamp": ts, "message": {"id": id, "content": [
                {"type": "tool_use", "name": "mcp__tasqx__tasqx_complete_task",
                 "input": {"ref": r}}]}})
            .to_string()
        };
        std::fs::write(
            proj.join("sess.jsonl"),
            [
                usage("2026-07-24T10:10:00Z", "m-a", 1),
                usage("2026-07-24T10:25:00Z", "m-both", 100),
                complete("2026-07-24T10:30:00Z", "c-a", a),
                usage("2026-07-24T10:40:00Z", "m-b", 10),
                complete("2026-07-24T10:50:00Z", "c-b", b),
            ]
            .join("\n"),
        )
        .unwrap();
        ...
        let got: Vec<(u64, &str)> = results
            .iter()
            .map(|r| {
                let r = r
                    .as_ref()
                    .expect("both tasks bank their uncontested samples");
                (r.totals.input, r.confidence)
            })
            .collect();
        assert_eq!(
            got,
            vec![(1, CONFIDENCE_HIGH), (10, CONFIDENCE_HIGH)],
            "the 10:25 sample inside both windows banks for neither"
        );
    }
